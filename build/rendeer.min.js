...
gl.ONE_MINUS_SRC_ALPHA):gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA),gl.depthMask(!1),q.u_alpha_cutoff=0):"MASK"==f.alphaMode?q.u_alpha_cutoff=f.alphaCutoff:gl.disable(gl.BLEND);if(b)if(b.constructor===m.Skeleton)s.setUniform("u_bones",b.computeFinalBoneMatrices(),g);else if(b._bone_matrices)s.setUniform("u_bones",b._bone_matrices);else return;1==v&&r._uniforms.u_model.set(e);b&&mat4.identity(r._uniforms.u_model);s.uniforms(r._uniforms);s.uniforms(this.global_uniforms);s.uniforms(f.uniforms);
