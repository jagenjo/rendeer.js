...
this._root.flags.no_transform=!0;this._root._scene=this;this._nodes_by_id={};this._nodes=[];this._to_destroy=[];this.frame=this.time=0}function g(d){this._color=vec4.fromValues(1,1,1,1);this.shader_name=null;this.uniforms={u_color:this._color};this.textures={};this.primitive=GL.TRIANGLES;this.blend_mode=b.BLEND_NONE;this.flags={two_sided:!1,depth_test:!0,depth_write:!0};d&&this.configure(d)}function f(d,b){b=b||{};var a=this.gl=this.context=d;if(!a||!a.enable)throw"litegl GL context not found.";d!=
...
b){if(this.children){var a=this.children.indexOf(d);-1!=a&&(this.children.splice(a,1),this.children.splice(b,0,d))}};a.prototype.getAllChildren=function(d){d=d||[];if(!this.children)return d;for(var b=0,a=this.children.length;b<a;b++){var c=this.children[b];d.push(c);c.getAllChildren(d)}return d};a.prototype.getVisibleChildren=function(d,b,a){d=d||[];void 0===b&&(b=65535);if(!this.children||!1===this.flags.visible)return d;for(var c=0,h=this.children.length;c<h;c++){var f=this.children[c];if(!1!==
f.flags.visible){var r=f.layers&b;if(!a||r)r&&d.push(f),f.getVisibleChildren(d,b)}}return d};a.prototype.serialize=function(){var d={position:[this._position[0],this._position[1],this._position[2]],rotation:[this._rotation[0],this._rotation[1],this._rotation[2],this._rotation[3]],scale:[this._scale[0],this._scale[1],this._scale[2]],children:[]};this.name&&(d.name=this.name);this.primitives&&this.primitives.length&&(d.primitives=this.primitives.map(function(d){return Object.assign({},d)}));this.mesh&&
...
!0};a.prototype.lookAt=function(d,b,a,c){this.position=d;this.orientTo(b,c,a)};a.prototype.orientTo=function(d,a,c,h,f){var r=this.getGlobalPosition(),e=vec3.create();h?e.set(d):vec3.sub(e,r,d);f&&(e[1]=0);c=c||b.UP;vec3.normalize(e,e);a&&vec3.scale(e,e,-1);d=mat3.create();c=vec3.cross(vec3.create(),c,e);vec3.normalize(c,c);a=vec3.cross(vec3.create(),e,c);vec3.normalize(a,a);mat3.setColumn(d,c,0);mat3.setColumn(d,a,1);mat3.setColumn(d,e,2);quat.fromMat3(this._rotation,d);quat.normalize(this._rotation,
...
function(d){if(null==d)return null;for(var b=0,a=this.children.length;b<a;b++){var c=this.children[b];if(c.name==d||(c=c.findNodeByName(d)))return c}return null};a.prototype.findNodesByFilter=function(d,b,a){void 0===b&&(b=65535);a=a||[];for(var c=0,h=this.children.length;c<h;c++){var f=this.children[c];f.layers&b&&(d&&!d(f)||a.push(f),f.findNodesByFilter(d,b,a))}return a};a.prototype.propagate=function(d,b){for(var a=0,c=this.children.length;a<c;a++){var h=this.children[a];h&&(h[d]&&h[d].apply(h,
...
mat4.create();return function(b,a,c,h,f,e){c=void 0===c?Number.MAX_VALUE:c;void 0===h&&(h=65535);a=a||vec3.create();void 0!==m._ray_tested_objects&&m._ray_tested_objects++;var r=null,n=null;if(!1===this.flags.visible)return null;this.layers&h&&!this.flags.ignore_collisions&&this.mesh&&(n=this.testRayWithMesh(b,d,c,h,f,e));n&&(e=vec3.distance(b.origin,d),null==c||e<c)&&(c=e,a.set(d),r=this);if(!this.children||!this.children.length)return r;for(var n=vec3.create(),q=0,s=this.children.length;q<s;++q){var g=
this.children[q].testRay(b,n,c,h,f);g&&(e=vec3.distance(b.origin,n),e>c||(c=e,a.set(n),r=g))}return r}}();a.prototype.testRayWithMesh=function(){var d=vec3.create(),a=vec3.create(),c=vec3.create(),h=mat4.create();return function(f,e,r,n,q){if(!this.mesh)return!1;var s=gl.meshes[this.mesh];if(!s||!1===s.ready)return!1;var g=null==this.submesh?-1:this.submesh,l=this._global_matrix;mat4.invert(h,l);vec3.transformMat4(d,f.origin,h);vec3.add(c,f.origin,f.direction);vec3.transformMat4(c,c,h);vec3.sub(a,
c,d);vec3.normalize(a,a);var k=this.flags.two_sided;if(this.primitives&&this.primitives.length){var A=b.Materials[this.primitives[0].material];A&&(k=A.flags.two_sided)}return b.testRayMesh(f,d,a,l,s,g,e,r,n,q,k)}}();a.prototype.setRangeFromSubmesh=function(d){if(void 0!==d&&this.mesh){var b=gl.meshes[this.mesh];if(b&&b.info&&b.info.groups){if(d.constructor===String){for(var a=0;a<b.info.groups.length;++a)if(b.info.groups[a].name==d){d=a;break}if(a===b.info.groups.length)return!1}if(d=b.info.groups[d])this.draw_range[0]=
d.start,this.draw_range[1]=d.length}else console.warn("you cannot set the submesh_id while the mesh is not yet loaded")}else this.draw_range=null};a.prototype.findNodesInSphere=function(d,b,a,c){void 0===a&&(a=65535);c=c||[];for(var h=0;h<this.children.length;++h){var f=this.children[h];f.layers&a&&(f.getGlobalPosition(u,!0),vec3.distance(u,d)<=b&&c.push(f));f.children.length&&f.findNodesInSphere(d,b,a,c)}return c};b.Camera=k;k.PERSPECTIVE=1;k.ORTHOGRAPHIC=2;k.prototype.configure=function(d){null!=
...
vec3.create(),d,this._viewprojection_matrix,b)};k.prototype.getRay=function(d,a,c,h){if(void 0===d||void 0===a)throw"RD.Camera.getRay requires x and y parameters";c=c||gl.viewport_data;h||(h=new b.Ray);this._must_update_matrix&&this.updateMatrices();var f=h.origin;vec3.set(f,d,a,0);this.type==b.Camera.ORTHOGRAPHIC?vec3.unproject(f,f,this._viewprojection_matrix,c):vec3.copy(f,this.position);var e=h.direction;vec3.set(e,d,a,1);vec3.unproject(e,e,this._viewprojection_matrix,c);vec3.sub(e,e,f);vec3.normalize(e,
e);return h};k.prototype.getRayPlaneCollision=function(d,b,a,c,h,f){h=h||vec3.create();d=this.getRay(d,b,f);return geo.testRayPlane(d.origin,d.direction,a,c,h)?h:null};k.prototype.getModelForScreenPixel=function(d,b,a,c,h){h=h||mat4.create();d=this.unproject([d,b,-1]);b=vec3.sub(vec3.create(),d,this._position);vec3.normalize(b,b);vec3.scaleAndAdd(d,d,b,a);vec3.normalize(b,b);mat4.fromTranslationFrontTop(h,d,b,this._up);return h};k.controller_keys={forward:"UP",back:"DOWN",left:"LEFT",right:"RIGHT"};
k.prototype.applyController=function(d,a,c,h){c=c||10;if(d){var f=vec3.create();if(gl.keys[k.controller_keys.forward]||h&&gl.keys.W)f[2]=-1;else if(gl.keys[k.controller_keys.back]||h&&gl.keys.S)f[2]=1;if(gl.keys[k.controller_keys.left]||h&&gl.keys.A)f[0]=-1;else if(gl.keys[k.controller_keys.right]||h&&gl.keys.D)f[0]=1;vec3.sqrLen(f)&&this.moveLocal(f,d*c)}a&&(a.deltax&&this.rotate(-0.005*a.deltax,b.UP),a.deltay&&this.rotateLocal(-0.005*a.deltay,b.RIGHT))};k.prototype.lerp=function(d,b){vec3.lerp(this._position,
...
function(d,a,c,h,f){b.Scene._ray_tested_objects=0;a||(a=d.collision_point);void 0===f&&(f=!0);return this.root.testRay(d,a,c,void 0===h?65535:h,f)};b.testRayWithNodes=function(d,a,c,h,f,e,r){b.testRayWithNodes.coll_node=null;h=null==h?Number.MAX_VALUE:h;f=void 0===f?65535:f;b.Scene._ray_tested_objects=0;c||(c=d.collision_point);b.testRayWithNodes.local_result||(b.testRayWithNodes.local_result=vec3.create());for(var n=b.testRayWithNodes.local_result,q=null,s=0;s<a.length;++s){var g=a[s],l=g.testRay(d,
n,h,f,e);if(l){var k=vec3.distance(d.origin,n);k>h||(h=k,c.set(n),b.testRayWithNodes.coll_node=l,q=r?l:g)}}return q};b.last_hit_test=null;b.testRayMesh=function(d,a,c,h,f,e,r,n,q,s,g){n=null==n?Number.MAX_VALUE:n;var l=null;q=null;-1==e?(l=f.getBoundingBox(),q=f):(q=f.info.groups[e],l=q.bounding,l||(f.computeGroupsBoundingBoxes(),l=q.bounding));if(!l)return!1;n||(n=1E7);if(!geo.testRayBBox(a,c,l,null,u))return!1;vec3.transformMat4(r,u,h);e=vec3.distance(d.origin,r);if(e>n)return!1;if(!s)return!0;
q._octree||(q._octree=q==f?new GL.Octree(f):new GL.Octree(f,q.start,q.length));a=q._octree.testRay(a,c,0,n,g);if(!a)return!1;b.last_hit_test=a;r.set(a.hit);vec3.transformMat4(r,r,h);e=vec3.distance(d.origin,r);return e>n?!1:!0};m.prototype.fromJSON=function(d){this.root.clear();this.root.configure(d)};m.prototype.toJSON=function(d){function b(c,h){if(d){if(!d(c,h))return!1}else{if(!c.flags.no_transform){h.position=typedArrayToArray(c.position);if(0!=c.rotation[0]||0!=c.rotation[1]||0!=c.rotation[2]||
...
typedArrayToArray(this.emissive));this.model&&(d.model=this.model,d.metallicFactor=this.metallicFactor,d.roughnessFactor=this.roughnessFactor);return d};g.prototype.render=function(d,a,c,h,f){var e=this.shader_name;e||(e="pbrMetallicRoughness"==this.model?"texture_albedo":d.default_shader_name||b.Material.default_shader_name);(e=gl.shaders[e])||(e=this.textures.color||this.textures.albedo?d._texture_shader:d._flat_shader);var r=0,n=null,q;for(q in this.textures){var s=this.textures[q];if(s){s.constructor===
Object&&(s=s.texture);var l="u_"+q+"_texture";if(!e||e.samplers[l])n=gl.textures[s],n||(d.autoload_assets&&-1!=s.indexOf(".")&&d.loadTexture(s,d.default_texture_settings),n=gl.textures.white),this.uniforms[l]=n.bind(r++)}}d.enableItemFlags(this);d._uniforms.u_model.set(a);e.uniforms(d._uniforms);e.uniforms(this.uniforms);a=null;null!=f&&c.info&&c.info.groups&&c.info.groups[f]&&(a=c.info.groups[f]);a?e.drawRange(c,this.primitive,a.start,a.length,h):e.draw(c,this.primitive,h);d.disableItemFlags(this);
d.draw_calls+=1};b.Material=g;b.Renderer=f;Object.defineProperty(f.prototype,"color",{set:function(d){this._color.set(d)},get:function(){return this._color},enumerable:!0});f.prototype.setDataFolder=function(d){d?(this.assets_folder=d,"/"!=this.assets_folder.substr(-1)&&(this.assets_folder+="/")):this.assets_folder=""};f.prototype.clear=function(d){d?this.gl.clearColor(d[0],d[1],d[2],3<=d.length?d[3]:1):this.gl.clearColor(0,0,0,0);this.gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT)};f._sort_by_dist_func=
function(d,b){return b._distance-d._distance};f._sort_by_priority_func=function(d,b){return b.render_priority-d.render_priority};f._sort_by_priority_and_dist_func=function(d,b){var a=b.render_priority-d.render_priority;return 0!=a?a:b._distance-d._distance};f.prototype.render=function(d,a,c,h,f,e){null==h&&(h=65535);if(!d)throw"Renderer.render: scene not provided";if(this._current_scene)throw this._current_scene=null,"Cannot render an scene while rendering an scene";a=a||d.camera;if(!a)throw"Renderer.render: camera not provided";
...
this.sort_by_priority?c.sort(b.Renderer._sort_by_priority_func):this.sort_by_distance&&c.sort(b.Renderer._sort_by_dist_func);if(this.onPreRender)this.onPreRender(a);d._root.preRender&&d._root.preRender(this,a);if(f=f||this.pipeline)f.render(this,c,a,d,e);else{for(f=0;f<c.length;++f){e=c[f];e.updateGlobalMatrix(!0);if(this.onPreRenderNode)this.onPreRenderNode(e,a);e.preRender&&e.preRender(this,a)}for(f=0;f<c.length;++f)e=c[f],e.flags.was_rendered=!1,!1!==e.flags.visible&&e.layers&h&&(!this.mustRenderNode||
!1!==this.mustRenderNode(e,a))&&(e.flags.was_rendered=!0,this.setModelMatrix(e._global_matrix),e.render?e.render(this,a):this.renderNode(e,a));d._root.postRender&&d._root.postRender(this,a);for(f=0;f<c.length;++f)if(e=c[f],e.postRender&&e.postRender(this,a),this.onPostRenderNode)this.onPostRenderNode(e,a)}if(this.onPostRender)this.onPostRender(a);d.frame++;this.frame++;this._current_scene=null};f.prototype.enableCamera=function(d){this._camera=d;d.updateMatrices();d.extractPlanes();this._view_matrix.set(d._view_matrix);
this._projection_matrix.set(d._projection_matrix);this._viewprojection_matrix.set(d._viewprojection_matrix);this._uniforms.u_camera_position=d.position};f.prototype.enable2DView=function(){mat4.ortho(this._viewprojection2D_matrix,0,gl.viewport_data[2],0,gl.viewport_data[3],-1,1)};f.prototype.saveState=function(){this.state.push({camera:this._camera,nodes:this._nodes})};f.prototype.restoreState=function(){var d=this.state.pop(),b=this.camera=d.camera;this._view_matrix.set(b._view_matrix);this._projection_matrix.set(b._projection_matrix);
this._viewprojection_matrix.set(b._viewprojection_matrix);this._uniforms.u_camera_position=b.position;this._nodes=d.nodes};f.prototype.setModelMatrix=function(d){this._model_matrix.set(d);mat4.multiply(this._mvp_matrix,this._viewprojection_matrix,d)};f.prototype.setGlobalUniforms=function(d){for(var b in d)this._uniforms[b]&&this._uniforms[b].set?this._uniforms[b].set(d[b]):this._uniforms[b]=d[b]};var E={u_model:null};f.prototype.renderNode=function(d,a){var c=null;d._mesh?c=d._mesh:d.mesh&&(c=gl.meshes[d.mesh],
c||(this._meshes_missing++,this.autoload_assets&&-1!=d.mesh.indexOf(".")&&this.loadMesh(d.mesh)));if(d.primitives&&d.primitives.length){if(c)for(var h=0;h<d.primitives.length;++h){var f=d.primitives[h],e=this.overwrite_material||b.Materials[f.material];!e||this.onFilterByMaterial&&!1==this.onFilterByMaterial(e,b.Materials[f.material])||this.renderMeshWithMaterial(d._global_matrix,c,e,"triangles",h)}}else{if(c&&(d.material||this.overwrite_material)&&(e=this.overwrite_material||b.Materials[d.material])){if(e.render){this.renderMeshWithMaterial(d._global_matrix,
c,e,d.indices,d.submesh);return}d.color=e.color}if(c){f=!1;d._instances&&(1<gl.webgl_version||gl.extensions.ANGLE_instanced_arrays)&&(f=!0);var e=null,r=d.shader;this.on_getShader?e=this.on_getShader(d,a):(!e&&d.shader&&(e=gl.shaders[r]),this.shader_overwrite&&(e=gl.shaders[this.shader_overwrite]));e||(e=d.textures.color?this._texture_shader:this._flat_shader);f&&!e.attributes.u_model&&(f=!1);var r=0,n=null;for(h in d.textures){var q=d.textures[h];if(q){q.constructor===Object&&(q=q.texture);var s=
"u_"+h+"_texture";if(!e||e.samplers[s])n=gl.textures[q],n||(this.autoload_assets&&-1!=q.indexOf(".")&&this.loadTexture(q,this.default_texture_settings),n=gl.textures.white),d.flags.pixelated?(n.bind(0),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST)):!1===d.flags.pixelated&&(n.bind(0),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR_MIPMAP_LINEAR)),
d._uniforms[s]=n.bind(r++)}}this.ignore_flags||this.enableItemFlags(d);if(d.onRender)d.onRender(this,a,e);for(h=0;h<this.global_uniforms_containers.length;++h)e.uniforms(this.global_uniforms_containers[h]);this.skip_node_uniforms||e.uniforms(d._uniforms);if(d.onShaderUniforms)d.onShaderUniforms(this,e);if(this.onNodeShaderUniforms)this.onNodeShaderUniforms(this,e,d);h=null;null!=d.submesh&&c.info&&c.info.groups&&c.info.groups[d.submesh]&&(h=c.info.groups[d.submesh]);f?(E.u_model=d._instances,h?e.drawInstanced(c,
void 0===d.primitive?gl.TRIANGLES:d.primitive,d.indices,E,h.start,h.length):d.draw_range?e.drawInstanced(c,void 0===d.primitive?gl.TRIANGLES:d.primitive,d.indices,E,d.draw_range[0],d.draw_range[1]):e.drawInstanced(c,void 0===d.primitive?gl.TRIANGLES:d.primitive,d.indices,E)):h?e.drawRange(c,void 0===d.primitive?gl.TRIANGLES:d.primitive,h.start,h.length,d.indices):d.draw_range?e.drawRange(c,void 0===d.primitive?gl.TRIANGLES:d.primitive,d.draw_range[0],d.draw_range[1],d.indices):e.draw(c,void 0===d.primitive?
gl.TRIANGLES:d.primitive,d.indices);this.ignore_flags||this.disableItemFlags(d);this.draw_calls+=1}else if(d.onRender)d.onRender(this,a)}};f.prototype.renderMesh=function(d,a,c,h,f,e,r,n){a&&(h&&this._uniforms.u_color.set(h),d||(d=b.IDENTITY),this._uniforms.u_model.set(d),f||(f=c?gl.shaders.texture:gl.shaders.flat),c&&(this._uniforms.u_texture=c.bind(0)),f.uniforms(this._uniforms),f.draw(a,void 0===e?gl.TRIANGLES:e,r),this.draw_calls+=1)};f.prototype.renderMeshWithMaterial=function(d,b,a,c,h){a.render&&
a.render(this,d,b,c,h)};f.prototype.renderBounding=function(d,a,c){if(d){a=a||b.IDENTITY;var h=this._uniforms.u_model,f=null,f=d.constructor===GL.Mesh?d._bounding:d;c=c||[1,1,0,1];d=f.subarray(3,6);mat4.translate(h,a,f.subarray(0,3));mat4.scale(h,h,[2*d[0],2*d[1],2*d[2]]);this.renderMesh(h,gl.meshes.cube,null,c,null,gl.LINES,"wireframe")}};f.prototype.enableItemFlags=function(d){var a=d.flags.flip_normals;this.reverse_normals&&(a=!a);gl.frontFace(a?gl.CW:gl.CCW);gl[!1===d.flags.depth_test?"disable":
...
gl.ONE_MINUS_SRC_ALPHA),gl.depthMask(!1))};f.prototype.disableItemFlags=function(d){d.flags.flip_normals&&gl.frontFace(gl.CCW);!1===d.flags.depth_test&&gl.enable(gl.DEPTH_TEST);d.blend_mode!==b.BLEND_NONE&&gl.disable(gl.BLEND);d.flags.two_sided&&gl.disable(gl.CULL_FACE);!1===d.flags.depth_write&&gl.depthMask(!0);"BLEND"==d.alphaMode&&(gl.depthMask(!0),gl.disable(gl.BLEND))};f.prototype.setPointSize=function(d){this.point_size=d;gl.shaders.point.uniforms({u_pointSize:this.point_size})};b.Renderer.prototype.renderPoints=
function(d,a,c,h,f,e,r,n,q){if(!d||d.constructor!==Float32Array)throw"RD.renderPoints only accepts Float32Array";f||(r==GL.LINES||r==GL.LINE_LOOP?f=this.shaders.flat:n?(f=this.shaders._points_textured,f||(f=this.shaders._points_textured=new GL.Shader(b.points_vs_shader,b.points_fs_shader,{TEXTURED:""}),f.uniforms({u_texture:0,u_atlas:1,u_pointSize:1}))):(f=this.shaders[a?"_points_color":"_points"],f||(f=a?this.shaders._points_color=new GL.Shader(b.points_vs_shader,b.points_fs_shader,{COLORED:""}):
this.shaders._points=new GL.Shader(b.points_vs_shader,b.points_fs_shader),f.uniforms({u_texture:0,u_atlas:1,u_pointSize:1}))));e=e||1;var s=1024;h=h||d.length/3;var l=null,g=null,k=this._points_mesh;h>d.length/3&&(h=d.length/3);!k||d.length>3*s?(h>s&&(s=GL.Texture.nextPOT(h)),l=new Float32Array(3*s),g=new Float32Array(4*s),k=this._points_mesh=GL.Mesh.load({vertices:l,extra4:g})):(l=this._points_mesh.getBuffer("vertices").data,g=this._points_mesh.getBuffer("extra4").data);l.set(l.length>d.length?d:
d.subarray(0,l.length));a?g.set(g.length>a.length?a:a.subarray(0,g.length)):g.fill&&g.fill(0);k.upload(GL.DYNAMIC_STREAM);f.setUniform("u_color",this._color);f.setUniform("u_pointSize",e);f.setUniform("u_camera_perspective",c._projection_matrix[5]);f.setUniform("u_model",q||b.IDENTITY);f.setUniform("u_viewport",gl.viewport_data);f.setUniform("u_viewprojection",c._viewprojection_matrix);n&&f.setUniform("u_texture",n.bind(0));f.drawRange(k,void 0!==r?r:GL.POINTS,0,h);return k};b.points_vs_shader="\nprecision highp float;\nattribute vec3 a_vertex;\nattribute vec4 a_extra4;\nvarying vec3 v_pos;\nvarying vec4 v_extra4;\nuniform mat4 u_model;\nuniform mat4 u_viewprojection;\nuniform vec4 u_viewport;\nuniform float u_camera_perspective;\nuniform float u_pointSize;\n\nfloat computePointSize( float radius, float w )\n{\n\tif(radius < 0.0)\n\t\treturn -radius;\n\treturn u_viewport.w * u_camera_perspective * radius / w;\n}\n\nvoid main() {\n\tv_pos = (u_model * vec4(a_vertex,1.0)).xyz;\n\tv_extra4 = a_extra4;\n\tgl_Position = u_viewprojection * vec4(v_pos,1.0);\n\tgl_PointSize = computePointSize( u_pointSize, gl_Position.w );\n}\n";
...
f.prototype.renderLines=function(d,a,c,h){if(!d||d.constructor!==Float32Array)throw"RD.renderPoints only accepts Float32Array";var f=this.shaders._lines;f||(f=this.shaders._lines=new GL.Shader(b.lines_vs_shader,this._flat_fragment_shader));var e=this._camera,r=1024,n=d.length/3,q=c?2*n:4*n,s=null,l=null,g=null,r=this._lines_mesh;!r||3*q>r.getBuffer("vertices").data.length?(r=GL.Texture.nextPOT(q),s=new Float32Array(3*r),l=new Float32Array(3*r),g=new Float32Array(2*r),indices_data=new Uint16Array(3*
...
d.subarray(3*v,3*v+3),v=d.subarray(3*v+3,3*v+6);vec3.sub(p,v,x);s.set(x,12*t);s.set(x,12*t+3);s.set(v,12*t+6);s.set(v,12*t+9);l.set(p,12*t);l.set(p,12*t+3);l.set(p,12*t+6);l.set(p,12*t+9);g.set(q,8*t);g.set(k,8*t+2);g.set(A,8*t+4);g.set(m,8*t+6);indices_data.set([4*t+0,4*t+2,4*t+1,4*t+1,4*t+2,4*t+3],6*t)}r.upload(GL.DYNAMIC_STREAM);gl.enable(gl.CULL_FACE);f.setUniform("u_model",h||b.IDENTITY);f.setUniform("u_color",this._color);f.setUniform("u_camera_front",e._front);f.setUniform("u_camera_position",
e.eye);f.setUniform("u_lineWidth",0.5*a);f.setUniform("u_camera_perspective",e._projection_matrix[5]);f.setUniform("u_viewport",gl.viewport_data);f.setUniform("u_viewprojection",e._viewprojection_matrix);f.drawRange(r,gl.TRIANGLES,0,6*n);return r};b.lines_vs_shader="\nprecision highp float;\nattribute vec3 a_vertex;\nattribute vec3 a_normal;\nattribute vec2 a_extra2;\nuniform mat4 u_model;\nuniform mat4 u_viewprojection;\nuniform vec4 u_viewport;\nuniform vec3 u_camera_front;\nuniform vec3 u_camera_position;\nuniform float u_camera_perspective;\nuniform float u_lineWidth;\n\nfloat computePointSize( float radius, float w )\n{\n\tif(radius < 0.0)\n\t\treturn -radius;\n\treturn u_viewport.w * u_camera_perspective * radius / w;\n}\nvoid main() {\n\tvec3 T = normalize( (u_model * vec4(a_normal,0.0)).xyz );\n\tvec3 pos = (u_model * vec4(a_vertex,1.0)).xyz;\n\tvec3 pos2 = (u_model * vec4(a_vertex + T,1.0)).xyz;\n\tvec3 front = u_camera_front;//normalize( a_vertex - u_camera_position );\n\tT = normalize( pos2 - pos ) ;\n\t//float proj_w = (u_viewprojection * vec4(a_vertex,1.0)).w;\n\t//float fixed_size_factor = computePointSize( u_lineWidth, proj_w );\n\tvec3 side = normalize( cross(T,front) * a_extra2.x ) * u_lineWidth;\n\tpos += side;\n\tgl_Position = u_viewprojection * vec4(pos,1.0);\n}\n";
f.prototype.getAssetFullPath=function(d,b){if(-1==d.indexOf("://")&&!b&&this.assets_folder){if(this.onGetAssetsFolder)return this.onGetAssetFolder(d);var a="/"==this.assets_folder.substr(-1),c="/"==d[0];return a!=c?this.assets_folder+d:a&&c?this.assets_folder+d.substr(1):this.assets_folder+"/"+d}return d};f.prototype.loadMesh=function(d,b,a){if(!d)return console.error("loadMesh: Cannot load null name");if(!this.assets_loading[d]&&!this.assets_not_found[d]){var c=this.meshes[d];if(c)return b&&b(c),
c;var h=this;a=this.getAssetFullPath(d,a);a=GL.Mesh.fromURL(a,function(a){a?h.meshes[d]=a:(h.assets_not_found[d]=!0,delete h.meshes[d]);h.num_assets_loading--;delete h.assets_loading[d];b&&b(a,d)});this.assets_loading[d]=a;this.num_assets_loading++;return this.meshes[d]=a}};f.prototype.loadTexture=function(d,b,a,c){function h(b){r.debug&&console.log(" + texture loaded: "+d);b?r.textures[f]=b:r.assets_not_found[d]=!0;a&&a(b,f);r.num_assets_loading--;delete r.assets_loading[d];if(r.on_texture_load)r.on_texture_load(b,
f)}if(!d)return console.error("loadTexture: Cannot load null name");if(!this.assets_loading[d]&&!this.assets_not_found[d]){var f=d;b&&(b.name&&(f=b.name),b.preview&&(f=b.preview));var e=this.textures[f];if(e&&!e.is_preview)return a&&a(e),e;var r=this;c=this.getAssetFullPath(d,c);var n=null;-1!=d.indexOf("CUBEMAP")?n=GL.Texture.cubemapFromURL(c,this.default_cubemap_settings,h):this.credentials?(this.credentials.headers?this.credentials.headers["Content-Type"]="application/octet-stream":this.credentials.headers=
{"Content-Type":"application/octet-stream"},n=new GL.Texture(1,1,b),fetch(c,this.credentials).then(function(b){if(!b.ok)throw Error("HTTP "+b.status+":"+b.statusText);return b.arrayBuffer()}).then(function(d){var a=URL.createObjectURL(new Blob([d]));b.texture=n;n=GL.Texture.fromURL(a,b,h);b.texture=null;setTimeout(function(){URL.revokeObjectURL(a)},6E4)})):n=GL.Texture.fromURL(c,b,h);b&&b.preview&&(n.is_preview=!0);this.assets_loading[d]=n;this.num_assets_loading++;return this.textures[f]=n}};f.prototype.loadTextureAtlas=
function(b,a,c){"string"==typeof b&&(b=JSON.parse(b));var h=this;-1==a.indexOf("://")&&(a=this.assets_folder+a);GL.Texture.fromURL(a,null,function(a){var f=b.files;h.textures[":atlas"]=a;for(var e in f)if(!h.textures[e]||h.textures[e].is_preview){var r=f[e],n=new GL.Texture(b.size,b.size,{wrap:gl.REPEAT,filter:gl.LINEAR});n.drawTo(function(){a.gl.drawTexture(a,0,0,b.size,b.size,r.x,r.y,r.width||b.size,r.height||b.size)});n.is_preview=!0;h.textures[e]=n}c&&c(f)})};f.prototype.loadShaders=function(b,
a,c,h){var f=this;-1!=b.indexOf("://")||h||(b=this.assets_folder+b);b+="?nocache="+Math.random();this.loading_shaders=!0;GL.loadFileAtlas(b,function(b){f.compileShadersFromAtlas(b,c);f.loading_shaders=!1;a&&a(b)})};f.prototype.reloadShaders=function(b){};f.prototype.compileShadersFromAtlasCode=function(b,a){var c=GL.processFileAtlas(b);this.compileShadersFromAtlas(c,a)};f.prototype.compileShadersFromAtlas=function(b,a){var c=b.shaders;if(c){this.shader_files=b;for(var h in b)b[h]=GL.Shader.expandImports(b[h],
b);c=c.split("\n");for(h=0;h<c.length;++h){var f=c[h].trim().split(" "),e=f[0].trim();if("//"!=e.substr(0,2)){var r=b[f[1]],n=b[f[2]],q=null,s={};if(3<f.length)for(var l=3;l<f.length;++l)if("#"==f[l][0])s[f[l].substr(1)]=!0;else{q=f.slice(l).join(" ");break}if(!s.WEBGL1||1==gl.webgl_version)if(!s.WEBGL2||2==gl.webgl_version){f[1]&&"@"==f[1][0]&&(s=f[1].substr(1)+"_VERTEX_SHADER",GL.Shader[s]&&(r=GL.Shader[s]));f[2]&&"@"==f[2][0]&&(s=f[2].substr(1)+"_FRAGMENT_SHADER",GL.Shader[s]&&(n=GL.Shader[s]));
if(q)try{q=JSON.parse(q)}catch(g){console.error("Error in shader macros: ",e,q,g)}if(q&&a){var s={},k;for(k in q)s[k]=q[k];for(k in a)s[k]=a[k];q=s}else a&&(q=a);try{r&&n?this.shaders[e]?this.shaders[e].updateShader(r,n,q):this.shaders[e]=new GL.Shader(r,n,q):console.warn("Shader subfile not found: ",f[1],f[2])}catch(A){GL.Shader.dumpErrorToConsole(A,r,n)}}}}}else console.warn("No 'shaders' found in shaders file atlas, check documentation")};f.prototype.setShadersFromFile=function(b){b=GL.processFileAtlas(b);
this.compileShadersFromAtlas(b)};f.cubemap_info=[{front:[1,0,0],up:[0,-1,0]},{front:[-1,0,0],up:[0,-1,0]},{front:[0,1,0],up:[0,0,1]},{front:[0,-1,0],up:[0,0,-1]},{front:[0,0,1],up:[0,-1,0]},{front:[0,0,-1],up:[0,-1,0]}];f.prototype.renderToCubemap=function(d,a,c,h,e,r,n){var q=f.cubemap_camera;q||(f.cubemap_camera=q=new b.Camera);q.perspective(90,1,r||0.1,n||1E3);var s=vec3.create(),l=this;d.drawTo(function(b,d){var r=f.cubemap_info[d];vec3.add(s,c,r.front);q.lookAt(c,s,r.up);l.clear();l.render(a,
q,h,e)});return d};f.prototype.drawSphere3D=function(b,a,c){gl.meshes.sphere||(gl.meshes.sphere=GL.Mesh.sphere({slices:32}));var h=gl.shaders.flat;h.setUniform("u_color",c);h.setUniform("u_viewprojection",this._viewprojection_matrix);mat4.identity(v);mat4.translate(v,v,b);mat4.scale(v,v,[a,a,a]);h.setUniform("u_model",v);h.draw(gl.meshes.sphere);this.draw_calls+=1};f.prototype.drawCircle2D=function(b,a,c,h,f){gl.meshes.circle||(gl.meshes.circle=GL.Mesh.circle({radius:1,slices:32}));gl.meshes.ring||
(gl.meshes.ring=GL.Mesh.ring({radius:1,thickness:0.02,slices:64}));var e=gl.shaders.flat;e.setUniform("u_color",h);e.setUniform("u_viewprojection",this._viewprojection2D_matrix);mat4.identity(v);mat4.translate(v,v,[b,a,0]);mat4.scale(v,v,[2*c,2*c,2*c]);e.setUniform("u_model",v);e.draw(gl.meshes[f?"circle":"ring"]);this.draw_calls+=1};f.prototype.drawLine2D=function(d,a,c,h,f,e,r){var n=gl.meshes.plane;r=r||gl.shaders.flat;r.setUniform("u_color",e);r.setUniform("u_viewprojection",this._viewprojection2D_matrix);
var q=c-d,s=h-a;e=Math.atan2(q,s);q=Math.sqrt(q*q+s*s);f/=2;mat4.identity(v);mat4.translate(v,v,[0.5*(d+c),0.5*(a+h),0]);mat4.rotate(v,v,e,b.FRONT);d=f;mat4.scale(v,v,[d,q,d]);r.setUniform("u_model",v);r.draw(n);this.draw_calls+=1};b.sortByDistance=function(b,a){b.forEach(function(b){b._distance=b.getDistanceTo(a)});b.sort(function(b,d){return d._distance-b._distance})};b.noBlending=function(d){return d.blend_mode===b.BLEND_NONE};b.generateTextureAtlas=function(b,a,c,h,f){a=a||1024;c=c||1024;h=h||
64;var e=0,r;for(r in b)e++;gl.disable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.disable(gl.BLEND);var e=new GL.Texture(a,c),n={width:a,height:c,size:h,files:{}},q=0,s=0,l={};e.drawTo(function(){for(var e in b)if(":"!=e[0]&&"white"!=e&&"black"!=e&&"notfound"!=e){var r=b[e];if(!r.is_preview&&r.texture_type==gl.TEXTURE_2D){if(f){var g=r.toBase64().hashCode();if(l[g]){n.files[e]=n.files[l[g]];continue}l[g]=e}n.files[e]={x:q,y:s};r.renderQuad(q,s,h,h);q+=h;if(q==a&&(q=0,s+=h,s==c)){console.warn("Atlas too small, some textures wont be stored.");
break}}}});e.info=n;console.log(n);return e};f.prototype.computeResourcesLoaded=function(b){var a=0,c;for(c in b){var h=b[c],f=this.textures[h];f&&!1===f.ready||(h=this.meshes[h])&&!1===h.ready||(f||h)&&a++}return a};Object.defineProperty(f.prototype,"meshes",{get:function(){return this.gl.meshes},set:function(b){},enumerable:!0});Object.defineProperty(f.prototype,"textures",{get:function(){return this.gl.textures},set:function(b){},enumerable:!0});Object.defineProperty(f.prototype,"shaders",{get:function(){return this.gl.shaders},
set:function(b){},enumerable:!0});f.prototype.addMesh=function(b,a){a.gl!=this.gl&&(a=a.cloneShared(this.gl));this.gl.meshes[b]=a};b.Renderer=f;b.Ray=l;l.prototype.testPlane=function(b,a){return geo.testRayPlane(this.origin,this.direction,b,a,this.collision_point)};l.prototype.testSphere=function(b,a,c){return geo.testRaySphere(this.origin,this.direction,b,a,this.collision_point,c)};l.prototype.closestPointOnRay=function(b,a,c){c=c||vec3.create();var h=vec3.create();vec3.add(h,this.origin,this.direction);
var f=vec3.create();vec3.add(f,b,a);geo.closestPointBetweenLines(this.origin,h,b,f,null,c);return c};b.Factory=function(d,a,c){d=b.Factory.templates[d];var h=new b.SceneNode;d&&h.configure(d);a&&(a.constructor===b.Scene?a.root:a).addChild(h);c&&h.configure(c);return h};b.Factory.templates={grid:{mesh:"grid",primitive:1,color:[0.5,0.5,0.5,0.5],blend_mode:b.BLEND_ALPHA},mesh:{shader:"phong"},sphere:{mesh:"sphere",shader:"phong"},floor:{mesh:"planeXZ",scaling:10,shader:"phong"}};e.prototype._ctor=function(){a.prototype._ctor.call(this);
this.vertices=[];this.normals=[];this.coords=[];this.indices=[];this._vertices_data=new Float32Array(3072);this._indices_data=this._coords_data=this._normals_data=null;this._total_indices=this._total=0;this._mesh=GL.Mesh.load({vertices:this._vertices_data})};e.prototype.updateVertices=function(b){b&&(this.vertices=b);this._total=this.vertices.length;this._vertices_data.length<this.vertices.length&&(this._vertices_data=new Float32Array(2*this.vertices.length),this._mesh.getBuffer("vertices").data=
this._vertices_data);this._vertices_data.set(this.vertices);this._mesh.getBuffer("vertices").upload(GL.STREAM_DRAW)};e.prototype.updateNormals=function(b){b&&(this.normals=b);if(!this._normals_data||this._normals_data.length<this.normals.length)this._normals_data=new Float32Array(2*this.normals.length),this._mesh.getBuffer("normals")||this._mesh.createVertexBuffer("normals",null,3,this._normals_data,GL.STREAM_DRAW);this._normals_data.set(this.normals);this._mesh.getBuffer("normals").upload(GL.STREAM_DRAW)};
e.prototype.updateCoords=function(b){b&&(this.coords=b);if(!this._coords_data||this._coords_data.length<this.normals.length)this._coords_data=new Float32Array(2*this.coords.length),this._mesh.getBuffer("coords")||this._mesh.createVertexBuffer("coords",null,2,this._coords_data,GL.STREAM_DRAW);this._coords_data.set(this.coords);this._mesh.getBuffer("coords").upload(GL.STREAM_DRAW)};e.prototype.updateIndices=function(b){b&&(this.indices=b);if(!this._indices_data||this._indices_data.length<this.indices.length)this._indices_data=
new Float32Array(2*this.indices.length),this._mesh.getIndexBuffer("triangles")||this._mesh.createIndicesBuffer("triangles",this._indices_data,GL.STREAM_DRAW);this._indices_data.set(this.indices);this._mesh.getIndexBuffer("triangles").upload(GL.STREAM_DRAW);this._total_indices=b.length};e.prototype.render=function(b,a){if(this._total){var c=b.shaders[this.shader||"flat"];if(c){b.setModelMatrix(this._global_matrix);var h=this._mesh,f=this._total_indices?this._total_indices:this._total/3;b.enableItemFlags(this);
c.uniforms(b._uniforms).uniforms(this._uniforms).drawRange(h,void 0===this.primitive?GL.TRIANGLES:this.primitive,0,f,this._total_indices?"triangles":null);b.disableItemFlags(this)}}};extendClass(e,a);b.DynamicMeshNode=e;t.prototype._ctor=function(){a.prototype._ctor.call(this);this.mesh="plane";this.size=vec2.fromValues(0,0);this.sprite_pivot=b.TOP_LEFT;this.blend_mode=b.BLEND_ALPHA;this.flags.two_sided=!0;this.flags.flipX=!1;this.flags.flipY=!1;this.flags.pixelated=!1;this.shader="texture_transform";
...
num_columns=b;var f=b=0,e=1/num_columns,r=1/h;h*=num_columns;if(!a){a=[];for(var n=0;n<h;++n)a.push(String(n))}for(n=0;n<a.length&&!(c[a[n]]={pos:[b,f],size:[e,r],normalized:!0},b+=e,1<=b&&(b=0,f+=r),1<=f);++n);return c};t.prototype.createFrames=function(b,a){t.createFrames(b,a,this.frames)};t.prototype.addFrame=function(b,a,c,h,f,e){this.frames[b]={pos:vec2.fromValues(a,c),size:vec2.fromValues(h,f),normalized:!!e}};t.prototype.updateTextureMatrix=function(b){mat3.identity(this.texture_matrix);if(!this.texture)return!1;
...
z)),!0;if(h.normalized)z[0]=this.flags.flipX?h.pos[0]+h.size[0]:h.pos[0],z[1]=1-h.pos[1]-h.size[1],mat3.translate(b,b,z),z[0]=h.size[0]*(this.flags.flipX?-1:1),z[1]=h.size[1];else{var f=a.height;z[0]=(this.flags.flipX?h.pos[0]+h.size[0]:h.pos[0])/a.width;z[1]=(f-h.pos[1]-h.size[1])/f;mat3.translate(b,b,z);z[0]=h.size[0]*(this.flags.flipX?-1:1)/a.width;z[1]=h.size[1]/a.height}mat3.scale(b,b,z);return!0};t.prototype.render=function(a,c){if(this.texture&&this.updateTextureMatrix(a)){var h=a.textures[this.texture];
if(h){this.flags.pixelated&&(h.bind(0),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,this.flags.pixelated?gl.NEAREST:gl.LINEAR),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,this.flags.pixelated?gl.NEAREST_MIPMAP_NEAREST:gl.LINEAR_MIPMAP_LINEAR));this.billboard_mode&&b.orientNodeToCamera(this.billboard_mode,this,c,a);0==this.size[0]&&!1!==h.ready&&(this.size[0]=h.width);0==this.size[1]&&!1!==h.ready&&(this.size[1]=h.height);var f=this.size,e=0,r=0;v.set(this._global_matrix);var n=!1;
this.current_frame&&this.current_frame.size&&(f=this.current_frame.size,n=this.current_frame.normalized);if(this.sprite_pivot){switch(this.sprite_pivot){case b.TOP_LEFT:e=0.5;r=-0.5;break;case b.TOP_CENTER:r=-0.5;break;case b.TOP_RIGHT:e=-0.5;break;case b.BOTTOM_LEFT:r=e=0.5;break;case b.BOTTOM_CENTER:r=0.5;break;case b.BOTTOM_RIGHT:e=-0.5,r=0.5}mat4.translate(v,v,[e*h.width*f[0],r*h.height*f[1],0])}n?mat4.scale(v,v,[h.width*f[0],h.height*f[1],1]):mat4.scale(v,v,[this.size[0],this.size[1],1]);a.setModelMatrix(v);
a.renderNode(this,a,c)}}};extendClass(t,a);b.Sprite=t;h.prototype._ctor=function(){this.mesh="cube";this.shader="skybox";this.scaling=[10,10,10];this.flags.depth_test=!1;this.flags.two_sided=!0};h.prototype.render=function(b,a){this.position=a.position;this.updateGlobalMatrix(!0);b.setModelMatrix(this._global_matrix);b.renderNode(this,a)};extendClass(h,a);b.Skybox=h;b.parseTextConfig=function(b){function a(b,d){for(var h=c.shift();h;)if(0==h.trim().length)h=c.shift();else{for(var f=0;"\t"==h[f]&&
f<h.length;)f++;if(f<d)break;var e={children:[]};try{var r=h.trim();-1!=r.indexOf(":")&&(r="{"+r+"}");var n=new Function("return "+r);e.data=n()}catch(q){console.error(q)}f>=d&&(b&&b.children.push(e),h=a(e,f+1))}return h}var c=b.split("\n");b={data:"",children:[]};a(b,0);return b};f.prototype.createShaders=function(){gl._shaders_created&&(this._flat_shader=gl.shaders.flat,this._flat_instancing_shader=gl.shaders.flat_instancing,this._flat_skinning_shader=gl.shaders.flat_skinning,this._point_shader=
...
gl._shaders_created=!0};b.orientNodeToCamera=function(a,c,h,f){if(a){if(a==b.BILLBOARD_CYLINDRIC||a==b.BILLBOARD_PARALLEL_CYLINDRIC){var e=null;a==b.BILLBOARD_CYLINDRIC?(e=c.getGlobalPosition(x),vec3.sub(u,h._position,e),z[0]=u[0],z[1]=u[2]):(z[0]=h._front[0],z[1]=h._front[2]);a=vec2.computeSignedAngle(z,b.FRONT2D);isNaN(a)||(mat4.rotateY(v,q,-a),c._global_matrix.set(v),mat4.setTranslation(c._global_matrix,c._position),mat4.scale(c._global_matrix,c._global_matrix,c._scale))}else a==b.BILLBOARD_PARALLEL_SPHERIC?
(c._global_matrix.set(h._model_matrix),mat4.setTranslation(c._global_matrix,c._position)):(mat4.lookAt(c._global_matrix,c._position,h.position,b.UP),mat4.invert(c._global_matrix,c._global_matrix)),mat4.scale(c._global_matrix,c._global_matrix,c._scale);f.setModelMatrix(c._global_matrix)}};b.readPixels=function(b,a){var c=new Image;c.src=b;c.onload=function(){var b=document.createElement("canvas");b.width=this.width;b.height=this.height;var d=b.getContext("2d");d.drawImage(this,0,0);b=d.getImageData(0,
0,b.width,b.height);a(b,this)}};b.alignDivToNode=function(b,a,c,h,f,e){function r(b){return 1E-5>Math.abs(b)?0:b}function n(b){return"matrix3d("+r(b[0])+","+r(-b[1])+","+r(b[2])+","+r(b[3])+","+r(b[4])+","+r(-b[5])+","+r(b[6])+","+r(b[7])+","+r(b[8])+","+r(-b[9])+","+r(b[10])+","+r(b[11])+","+r(b[12])+","+r(-b[13])+","+r(b[14])+","+r(b[15])+")"}var q=parseInt(b.clientWidth),s=parseInt(b.clientHeight),l=0.5*q,g=0.5*s,k=f._projection_matrix[5]*g;b.style.width=gl.canvas.width+"px";b.style.height=gl.canvas.height+
"px";b.style.perspective=k+"px";b.style.pointerEvents="none";a&&(b="translateZ("+k+"px) "+n(f._view_matrix)+" translate("+l+"px,"+g+"px)",a.style.transformStyle="preserve-3d",a.style.transform=b,a.style.width=q+"px",a.style.height=s+"px",a.style.pointerEvents="none");h=h.getGlobalMatrix();q=1/c.clientWidth;s=1/c.clientHeight;c.parentNode!=a&&a.appendChild(c);c.style.pointerEvents=e?"auto":"none";c.style.transform=function(b){return"translate(-50%,-50%) "+("matrix3d("+r(b[0])+","+r(b[1])+","+r(b[2])+
","+r(b[3])+","+r(-b[4])+","+r(-b[5])+","+r(-b[6])+","+r(-b[7])+","+r(b[8])+","+r(b[9])+","+r(b[10])+","+r(b[11])+","+r(b[12])+","+r(b[13])+","+r(b[14])+","+r(b[15])+")")}(h)+" scale3D("+q+","+s+",1)"};"undefined"==typeof GL&&(mat4.rotateVec3=function(b,a,c){var h=c[0],f=c[1];c=c[2];b[0]=a[0]*h+a[4]*f+a[8]*c;b[1]=a[1]*h+a[5]*f+a[9]*c;b[2]=a[2]*h+a[6]*f+a[10]*c;return b},mat4.projectVec3=function(b,a,c){var h=c[0],f=c[1];c=c[2];var e=a[1]*h+a[5]*f+a[9]*c+a[13],r=a[2]*h+a[6]*f+a[10]*c+a[14],n=a[3]*
h+a[7]*f+a[11]*c+a[15];b[0]=((a[0]*h+a[4]*f+a[8]*c+a[12])/n+1)/2;b[1]=(e/n+1)/2;b[2]=(r/n+1)/2;return b})})("undefined"!=typeof window?window:"undefined"!=typeof self?self:global);
...
RD.SceneNode);RD.PointCloud=a;a.prototype.render=function(f,l){if(0!=this.points.length){this.updateVertices();var e=this._meshes[f.gl.context_id];e||(e=new GL.Mesh(void 0,void 0,f.gl),this._vertices_buffer=e.createVertexBuffer("vertices",null,3,this._vertices,gl.DYNAMIC_DRAW),this._extra_buffer=e.createVertexBuffer("extra3",null,3,this._extra,gl.DYNAMIC_DRAW));this._mesh=e;this._vertices_buffer.uploadRange(0,12*this.points.length);this._extra_buffer.uploadRange(0,12*this.points.length);e=gl.shaders[this.shader];
e||(e=gl.shaders.pointcloud)||(gl.shaders.pointcloud=new GL.Shader(a._vertex_shader,a._pixel_shader));this.draw_range[1]=this.points.length;e=gl.getViewport();this._uniforms.u_pointSize=this.points_size/(gl.canvas.width/e[2]);this._uniforms.u_color=this.color;0<this.num_textures?(this._uniforms.u_texture_info[0]=1/this.num_textures,this._uniforms.u_texture_info[1]=this.num_textures*this.num_textures):this._uniforms.u_texture_info[0]=0;this.ignore_transform&&(mat4.identity(f._model_matrix),f._mvp_matrix.set(f._viewprojection_matrix));
f.renderNode(this,f,l)}};a.prototype.updateVertices=function(a){if(a=this.points.length)for(var l=this._vertices,e=this._extra,g=0,h=this.num_textures*this.num_textures,c=0;c<a;c++){var n=this.points[c];l.set(n.pos?n.pos:n,g);e[g]=1;e[g+1]=1;1<h&&(e[g+2]=n.tex);g+=3}};a._vertex_shader="\t\t\tprecision highp float;\t\t\tattribute vec3 a_vertex;\t\t\tattribute vec3 a_extra3;\t\t\tvarying vec2 v_coord;\t\t\tvarying vec4 v_color;\t\t\tvarying vec3 v_position;\t\t\tuniform vec3 u_camera_position;\t\t\tuniform mat4 u_mvp;\t\t\tuniform mat4 u_model;\t\t\tuniform vec4 u_color;\t\t\tuniform float u_pointSize;\t\t\tuniform vec2 u_texture_info;\t\t\tvoid main() {\n\t\t\t\tv_color = u_color;\n\t\t\t\tv_color.a *= a_extra3.y;\n\t\t\t\tv_coord.x = (a_extra3.z * u_texture_info.y) * u_texture_info.x;\n\t\t\t\tv_coord.y = abs(floor(v_coord.x) * u_texture_info.x);\n\t\t\t\tv_coord.x = fract(v_coord.x);\n\t\t\t\tgl_Position = u_mvp * vec4(a_vertex,1.0);\n\t\t\t\tv_position = (u_model * vec4(a_vertex,1.0)).xyz;\n\t\t\t\tfloat dist = distance( u_camera_position, v_position );\n\t\t\t\tgl_PointSize = 10.0 * u_pointSize / dist;\n\t\t\t}\t\t\t";
...
extendClass(k,RD.SceneNode);RD.ParticlesEmissor=k;k.prototype.update=function(a){var l=this.particles.length,e=this.particles_damping,g=this.particles_acceleration,h=vec3.length(g);if(l){for(var c=[],n=0;n<l;n++){var b=this.particles[n];vec3.scaleAndAdd(b.pos,b.pos,b.vel,a);h&&vec3.scaleAndAdd(b.vel,b.vel,g,a);e&&vec3.scaleAndAdd(b.vel,b.vel,b.vel,-a*e);b.ttl-=a;0<b.ttl&&c.push(b)}this.particles=c}l=this.getGlobalPosition();e=this.emissor_direction;g=this.particles_life;h=this.num_textures*this.num_textures;
c=this.velocity_variation;if(0<this.particles_per_second)for(a=this.particles_per_second*(a+this._accumulated_time),this._accumulated_time=(a-Math.floor(a))/this.particles_per_second,a=Math.floor(a),n=0;n<a&&!(this.particles.length>=this.max_particles);n++)e=vec3.clone(e),e[0]+=0.5*Math.random()*c,e[2]+=0.5*Math.random()*c,this.particles.push({id:this._last_particle_id++,tex:Math.floor(Math.random()*h)/h,pos:vec3.clone(l),vel:e,ttl:g})};k.prototype.render=function(a,g){if(0!=this.particles.length){this.updateVertices();
var e=this._meshes[a.gl.context_id];e||(e=new GL.Mesh(void 0,void 0,a.gl),this._vertices_buffer=e.createVertexBuffer("vertices",null,3,this._vertices,gl.DYNAMIC_DRAW),this._extra_buffer=e.createVertexBuffer("extra3",null,3,this._extra,gl.DYNAMIC_DRAW));this._mesh=e;this._vertices_buffer.uploadRange(0,12*this.particles.length);this._extra_buffer.uploadRange(0,12*this.particles.length);e=gl.shaders[this.shader];e||(e=gl.shaders.particles)||(gl.shaders.particles=new GL.Shader(k._vertex_shader,k._pixel_shader));
this.draw_range[1]=this.particles.length;e=gl.getViewport();this._uniforms.u_pointSize=this.particles_size/(gl.canvas.width/e[2]);this._uniforms.u_color=this.color;this._uniforms.u_texture_info[0]=1/this.num_textures;this._uniforms.u_texture_info[1]=this.num_textures*this.num_textures;this._uniforms.u_scaleStartEnd[0]=this.particles_start_scale;this._uniforms.u_scaleStartEnd[1]=this.particles_end_scale;mat4.identity(a._model_matrix);a._mvp_matrix.set(a._viewprojection_matrix);a.renderNode(this,a,
g)}};k.prototype.updateVertices=function(a){if(a=this.particles.length)for(var g=this._vertices,e=this._extra,k=0,h=this.particles_life,c=this.num_textures*this.num_textures,n=0;n<a;n++){var b=this.particles[n];g.set(b.pos,k);e[k]=1;e[k+1]=b.ttl/h;1<c&&(e[k+2]=b.tex);k+=3}};k._vertex_shader="\t\t\tprecision highp float;\t\t\tattribute vec3 a_vertex;\t\t\tattribute vec3 a_extra3;\t\t\tvarying vec2 v_coord;\t\t\tvarying vec4 v_color;\t\t\tvarying vec3 v_position;\t\t\tuniform vec3 u_camera_position;\t\t\tuniform mat4 u_mvp;\t\t\tuniform mat4 u_model;\t\t\tuniform vec4 u_color;\t\t\tuniform float u_pointSize;\t\t\tuniform vec2 u_texture_info;\t\t\tuniform vec2 u_scaleStartEnd;\t\t\tvoid main() {\n\t\t\t\tv_color = u_color;\n\t\t\t\tv_color.a *= a_extra3.y;\n\t\t\t\tv_coord.x = (a_extra3.z * u_texture_info.y) * u_texture_info.x;\n\t\t\t\tv_coord.y = floor(v_coord.x) * u_texture_info.x;\n\t\t\t\tv_coord.x = fract(v_coord.x);\n\t\t\t\tgl_Position = u_mvp * vec4(a_vertex,1.0);\n\t\t\t\tv_position = (u_model * vec4(a_vertex,1.0)).xyz;\n\t\t\t\tfloat dist = distance( u_camera_position, v_position );\n\t\t\t\tgl_PointSize = mix(u_scaleStartEnd.y, u_scaleStartEnd.x, a_extra3.y) * 10.0 * u_pointSize / dist;\n\t\t\t}\t\t\t";
...
g,e,k,h){if(this.max_sprites<=this.index)console.warn("too many sprites in batch, increase size");else{var c=this.index;this.index+=1;this.positions.set(a,3*c);2==a.length&&(this.positions[3*c+2]=0);a=4*c;this.sprite_info[a]=g||0;this.sprite_info[a+1]=e?1:0;this.sprite_info[a+2]=null==k?1:k;this.sprite_info[a+3]=h||0;this.must_update_buffers=!0}};g.prototype.addData=function(a,g){if(this.max_sprites<=this.index)console.warn("too many sprites in batch, increase size");else{var e=this.index;this.index+=
1;this.positions.set(a,3*e);this.sprite_info.set(g,4*e);this.must_update_buffers=!0}};g.prototype.render=function(a,g){if(this.texture){var e=a.textures[this.texture];e&&this.flags.pixelated&&(e.bind(0),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,this.flags.pixelated?gl.NEAREST:gl.LINEAR),gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,this.flags.pixelated?gl.NEAREST_MIPMAP_NEAREST:gl.LINEAR_MIPMAP_LINEAR));this.renderSprites(e,g,this.size,this.atlas_size,this.color,this.mode)}};g.prototype.renderSprites=
function(a,l,e,k,h,c){if(this.index){var n=gl.shaders[this.shader];n||(n=gl.shaders[this.use_points?"point_sprites":"quad_sprites"]);n||(n=this.use_points?gl.shaders.point_sprites=new GL.Shader(g.point_sprites_vertex_shader,g.point_sprites_fragment_shader):gl.shaders.quad_sprites=new GL.Shader(g.quad_sprites_vertex_shader,g.quad_sprites_fragment_shader));c=c||0;if(this.use_points)n.uniforms(GFX.scene_renderer._uniforms),n.setUniform("u_pointSize",e),n.setUniform("u_atlas",k),n.setUniform("u_texture",
...
{};for(var h in c){var f=c[h],e={mask:255,pos:vec3.create(),pos2D:vec3.create(),radius:0.15,visible:!1,flag:a[f.toUpperCase()]};0==f.indexOf("move")&&(e.move=!0);0==f.indexOf("rotate")&&(e.rotate=!0);0==f.indexOf("scale")&&(e.scale=!0);e.color=b[f[f.length-1]];if(e.move||e.rotate)e.cursor="crosshair";this.actions[f]=e}this.actions.scalex.axis=this.actions.rotatex.axis=this.actions.movex.axis=vec3.fromValues(1,0,0);this.actions.scaley.axis=this.actions.rotatey.axis=this.actions.movey.axis=vec3.fromValues(0,
...
a.OBJECT_SPACE=1;a.WORLD_SPACE=2;p=mat4.create();var k=mat4.create(),m=mat4.create();mat4.create();mat4.create();mat4.create();var g=mat4.create(),f=mat4.create();mat4.create();var l=vec3.create(),e=vec3.create(),t=vec3.create(),h=vec4.create();vec4.create();mat4.translate(p,p,[1.1,0,0]);mat4.rotate(p,p,90*DEG2RAD,[0,0,-1]);mat4.translate(k,k,[0,1.1,0]);mat4.translate(m,m,[0,0,1.1]);mat4.rotate(m,m,90*DEG2RAD,[1,0,0]);p=mat4.create();mat4.scale(p,p,[-1,-1,-1]);a.full_viewport=null;var c=mat4.create(),
...
a.prototype.removeTargetsFromScene=function(){if(this.targets){for(var b=0;b<this.targets.length;++b){var a=this.targets[b];a.layers&this.layers&&a.parentNode&&a.parentNode.removeChild(a)}this.targets=null}};a.prototype.getTargetBaseNodes=function(){function b(a){return-1!=c.indexOf(a)?!0:a.parentNode?b(a.parentNode):!1}if(!this.targets||!this.targets.length)return null;for(var a=[],c=this.targets,h=0;h<c.length;++h){var f=c[h];f.layers&this.layers&&(b(f.parentNode)||a.push(f))}return a};a.prototype.applyTransformToTarget=
function(b,c){var h=this.getGlobalMatrix(mat4.create()),f=mat4.invert(mat4.create(),h),e=mat4.create(),n=this.getTargetBaseNodes();if(n){for(var g=[1,1,1],l=0;l<n.length;++l){var k=n[l];g[0]=0<=k.scaling[0]?1:-1;g[1]=0<=k.scaling[1]?1:-1;g[2]=0<=k.scaling[2]?1:-1;c?(mat4.multiply(e,b,k.matrix),k.fromMatrix(e)):(k.getGlobalMatrix(e),this.coordinates==a.WORLD_SPACE&&mat4.multiply(e,f,e),mat4.multiply(e,b,e),this.coordinates==a.WORLD_SPACE&&mat4.multiply(e,h,e),k.fromMatrix(e,!0));vec3.mul(k._scale,
k._scale,g);k.position=this.applySnap(k.position);if(this.grid_size){var m=quat.toEuler(vec3.create(),k.rotation);m[0]=15*Math.round(m[0]*RAD2DEG/15)*DEG2RAD;m[1]=15*Math.round(m[1]*RAD2DEG/15)*DEG2RAD;m[2]=15*Math.round(m[2]*RAD2DEG/15)*DEG2RAD;quat.fromEuler(k.rotation,m);quat.normalize(k.rotation,k.rotation)}}mat4.multiply(e,h,b);this.fromMatrix(e);this.scaling=[1,1,1];this.updateGizmo()}};a.prototype.applyTranslation=function(b,a){var c=mat4.create();mat4.translate(c,c,b);this.applyTransformToTarget(c,
a)};a.prototype.applyRotation=function(b,a,c){var h=mat4.create();if(c){var f=mat4.create();mat4.setTranslation(f,c);mat4.mul(h,h,f);mat4.rotate(h,h,b,a);mat4.setTranslation(f,[-c[0],-c[1],-c[2]]);mat4.mul(h,h,f)}else mat4.rotate(h,h,b,a);this.applyTransformToTarget(h)};a.prototype.applyScale=function(b,a){b.constructor===Number&&(b=[b,b,b]);var c=mat4.create();mat4.scale(c,c,b);this.applyTransformToTarget(c,a)};a.prototype.applySnap=function(b){if(!this.grid_size)return b;b[0]=Math.round(b[0]/this.grid_size)*
...
function(b){this._hover_action=null;var c=[b.canvasx,b.canvasy],h=1E5;b=null;for(var f in this.actions){var e=this.actions[f];if(e.visible){var n=vec2.distance(e.pos2D,c);n<e.radius*this.size&&n<h&&(h=n,b=f)}}b||(f=vec2.distance(this.center_2D,c),this.actions.rotatefront.visible&&10>Math.abs(f-0.5*this.size)?b="rotatefront":this.mode&a.ROTATE&&f<0.5*this.size&&(b="rotate"));return this._hover_action=b};a.prototype.cancel=function(){this._selected_action&&(this._selected_action=null,this.restoreTargetTransforms())};
...
y=this._selected_action;y&&(x=y);var B=y?this.actions[y]:null,w=this.mode;mat4.rotateVec3(e,g,RD.RIGHT);mat4.rotateVec3(t,g,RD.UP);mat4.rotateVec3(l,g,RD.FRONT);vec3.normalize(e,e);vec3.normalize(t,t);vec3.normalize(l,l);mat4.fromTranslationFrontTop(f,p,q,m);vec4.transformMat4(h,h,n._viewprojection_matrix);this.current_size=m=gl.canvas.width/gl.canvas.height*this.size*h[3]/(gl.canvas.width*z);mat4.scale(g,g,[m,m,m]);mat4.scale(f,f,[m,m,m]);var m=vec3.dot(u,e),z=vec3.dot(u,t),D=vec3.dot(u,l),E=vec3.dot(q,
...
2,a.INNERSPHERE_COLOR,!0),b.drawLine2D(this.center_2D[0],this.click_2D[1],this.center_2D[0],this.center_2D[1],4,a.INNERSPHERE_COLOR)):(this.drawRotateSphere(g,a.STATIC_SPHERE_COLOR),w&a.ROTATE&&"rotate"==x&&this.drawRotateSphere(g,a.SPHERE_COLOR),w&a.ROTATEFRONT&&(mat4.rotateX(c,f,Math.PI/2),u.setUniform("u_model",c),u.setUniform("u_color","rotatefront"==x?a.SELECTED_COLOR:a.RING_COLOR),u.draw(k),this.actions.rotatefront.visible=!0),0.1<Math.abs(m)&&w&a.ROTATEX&&(mat4.rotateZ(c,g,Math.PI/2),0>C&&
...
a.YZAXIS_COLOR,"moveyz"):this.drawFlat(c,"scaleyz"==x?a.SELECTED_COLOR:a.YZAXIS_COLOR,"scaleyz","circle")),w&a.DRAG&&this.drawInnerSphere(g,"drag"),w&a.SCALE&&this.drawInnerSphere(g,"scale"),gl.enable(gl.DEPTH_TEST))}}};a.prototype.drawArrow=function(b,c,h){var f=gl.shaders[this.shader],e=gl.meshes.cone,g=gl.meshes.cylinder,l=this._hover_action;mat4.translate(n,b,[0,1.1,0]);f.setUniform("u_model",n);f.setUniform("u_color",l==h?a.SELECTED_COLOR:c);f.draw(e);this.mode==a.MOVEALL?(mat4.translate(n,n,
[0,-0.4,0]),mat4.scale(n,n,[1,1,1])):(mat4.translate(n,n,[0,-0.15,0]),mat4.scale(n,n,[1,0.5,1]));f.setUniform("u_model",n);f.draw(g);this.toScreen(n,h,[0,0.6,0])};a.prototype.drawAxis=function(b,a){var c=gl.shaders[this.shader],h=gl.meshes.sphere;mat4.scale(n,b,[0.01,100,0.01]);c.setUniform("u_model",n);gl.enable(gl.DEPTH_TEST);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(!1);c.setUniform("u_color",[a[0],a[1],a[2],0.2]);gl.depthFunc(gl.GREATER);c.draw(h);gl.depthFunc(gl.LESS);
gl.disable(gl.BLEND);c.setUniform("u_color",a);c.draw(h);gl.depthMask(!0);gl.disable(gl.DEPTH_TEST);this.drawInnerSphere(b)};a.prototype.drawFlat=function(b,a,h,f){f=gl.meshes[f||"plane"];var e=gl.shaders[this.shader];mat4.scale(c,b,[0.25,0.25,0.25]);e.setUniform("u_color",a);e.setUniform("u_model",c);gl.enable(gl.BLEND);e.draw(f);gl.disable(gl.BLEND);this.toScreen(c,h)};a.prototype.drawRotator=function(b,a,h){var f=gl.meshes.quartertorus,e=gl.shaders[this.shader];mat4.scale(c,b,[0.9,0.9,0.9]);e.setUniform("u_color",
a);e.setUniform("u_model",c);e.draw(f);this.toScreen(c,h,[-0.75,0,0.75])};a.prototype.drawScale=function(b,h,f){var e=gl.meshes.cylinder,n=gl.meshes.sphere,g=gl.shaders[this.shader];g.setUniform("u_color",h);this.mode==a.SCALEALL?(mat4.translate(c,b,[0,0.5,0]),mat4.scale(c,c,[1,1,1])):(mat4.translate(c,b,[0,0.25,0]),mat4.scale(c,c,[1,0.5,1]));g.setUniform("u_model",c);g.draw(e);mat4.translate(c,b,[0,0.4,0]);this.mode==a.SCALEALL?mat4.scale(c,c,[0.1,0.1,0.1]):mat4.scale(c,c,[0.1,0.2,0.1]);g.setUniform("u_model",
c);g.draw(n);this.toScreen(c,f)};a.prototype.drawRotateSphere=function(b,a){var h=gl.shaders[this.shader],f=gl.meshes.sphere;gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);mat4.scale(c,b,[0.9,0.9,0.9]);h.setUniform("u_model",c);h.setUniform("u_color",a);h.draw(f);gl.disable(gl.BLEND)};a.prototype.drawInnerSphere=function(b,h){var f=gl.shaders[this.shader],e=gl.meshes.sphere,n=this._hover_action;mat4.scale(c,b,[0.1,0.1,0.1]);f.setUniform("u_model",c);f.setUniform("u_color",n==
h?a.SELECTED_COLOR:a.INNERSPHERE_COLOR);f.draw(e);"drag"==h&&(mat4.scale(c,b,[0.07,0.07,0.07]),f.setUniform("u_model",c),f.setUniform("u_color",n==h?a.INNERSPHERE_COLOR:[0,0,0,1]),f.draw(e));h&&this.toScreen(c,h)};a.prototype.renderOutline=function(b,c,h,f){if((f=f||this.targets)&&f.length){var e=this.layers;f=f.filter(function(b){return b.layers&e});var n=gl.viewport_data[2],g=gl.viewport_data[3];this._selection_buffer&&this._selection_buffer.width==n&&this._selection_buffer.height==g||(this._selection_buffer=
new GL.Texture(n,g,{magFilter:gl.NEAREST}));a.outline_material||(a.outline_material=new RD.Material({shader_name:"flat"}));var l=this.shader;this._selection_buffer.drawTo(function(){gl.clearColor(0,0,0,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);b.shader_overwrite=l;var a=b.onNodeShaderUniforms;b.onNodeShaderUniforms=function(b,a){a.setUniform("u_color",[1,1,1,1])};var e=b.pipeline;b.pipeline=null;b.overwrite_material=RD.Gizmo.outline_material;b.render(c,h,f);b.shader_overwrite=null;b.onNodeShaderUniforms=
a;b.pipeline=e;b.overwrite_material=null});var k=gl.shaders.outline;k||(k=gl.shaders.outline=GL.Shader.createFX("\t\t\tvec3 colorU = texture2D(u_texture, uv - vec2(0.0,u_res.y)).xyz;\n\t\t\tvec3 colorUL = texture2D(u_texture, uv - u_res).xyz;\n\t\t\tvec3 colorUR = texture2D(u_texture, uv + vec2(u_res.x,-u_res.y)).xyz;\n\t\t\tvec3 colorL = texture2D(u_texture, uv - vec2(u_res.x,0.0)).xyz;\n\t\t\tvec3 colorDL = texture2D(u_texture, uv - vec2(u_res.x,-u_res.y)).xyz;\n\t\t\tvec3 outline = abs(color.xyz - colorU) * 0.3 + abs(color.xyz - colorL) * 0.3;\n\t\t\toutline += (abs(color.xyz - colorUL) + abs(color.xyz - colorDL) + abs(color.xyz - colorUR)) * 0.1;\n\t\t\tcolor = vec4( clamp(outline,vec3(0.0),vec3(1.0)),1.0 );\n\t\t\t//color = texture2D(u_texture, uv);\n\t\t",
...
magFilter:GL.LINEAR,minFilter:GL.LINEAR_MIPMAP_LINEAR,wrap:GL.REPEAT,no_flip:!1},load:function(p,a,k,m){function g(b){function a(c){var h=this.buffer;h.data=c;h.dataview=new Uint8Array(c);b.length?g(b):f()}var c=b.pop(),h=t+"/"+c.uri;"blob:"==c.uri.substr(0,5)&&(h=c.uri);"data:"==c.uri.substr(0,5)?(h=_base64ToArrayBuffer(c.uri.substr(37)),a.call({buffer:c},h)):fetch(h).then(function(b){return b.arrayBuffer()}).then(a.bind({buffer:c}))}function f(){l.filename=e;l.folder=t;l.url=p;var b=RD.GLTF.parse(l),
c=b.serialize();RD.GLTF.prefabs[p]=c;a&&a(b)}if(!p)throw"url missing";var l=null,e="",t="";if(p.constructor===String){t=p.split("/");e=t.pop();k=k||e.split(".").pop().toLowerCase();var t=t.join("/"),h=new XMLHttpRequest;h.onload=function(){if(200!=this.status)console.error("GLTF not found",p),a&&a(null);else{var b=h.response;"gltf"==k?(l=b,g(l.buffers.concat())):"glb"==k&&((l=RD.GLTF.parseGLB(b))?f():console.error("error parsing GLB:",e))}};h.onerror=function(){console.error("Network Error");a&&a(null)};
m&&(h.onprogress=function(b){m(p,b.loaded,b.total)});h.responseType="gltf"==k?"json":"arraybuffer";h.open("GET",p);h.send()}else{var c=p;if(e=c.main){p=e;var n=c[e];if("glb"==n.extension)(l=RD.GLTF.parseGLB(n.data))&&f();else{l=n.data;for(n=0;n<l.buffers.length;++n){var b=l.buffers[n];"data:"==b.uri.substr(0,5)?b.data=_base64ToArrayBuffer(b.uri.substr(37)):b.data=c[b.uri].data;b.dataview=new Uint8Array(b.data)}f()}}}},parseGLB:function(p){var a=new Uint8Array(p),k=new DataView(p);if(1179937895!=k.getUint32(0,
!0))return console.error("incorrect gltf header"),null;k.getUint32(4,!0);k.getUint32(8,!0);for(var m=12,g=null,f=0;m<a.length;){var l=k.getUint32(m,!0),e=k.getUint32(m+4,!0),t=p.slice(m+8,m+8+l),m=m+(8+l);if(e==RD.GLTF.JSON_CHUNK){if(!("TextDecoder"in window))throw"Sorry, this browser does not support TextDecoder...";g=(new TextDecoder("utf-8")).decode(t);g=JSON.parse(g)}else e==RD.GLTF.BINARY_CHUNK?(l=g.buffers[f],l.data=t,l.dataview=new Uint8Array(t),f++):console.warn("gltf unknown chunk type: ",
"0x"+e.toString(16))}return g},parse:function(p,a){p.url||(p.url=a||"scene.glb");var k=null,m={};1<p.scenes.length&&console.warn("gltf importer only supports one scene per file, skipping the rest");var g=p.scenes[p.scene].nodes;this.gltf_materials={};if(p.skins)for(var f=0;f<p.skins.length;++f)for(var k=p.skins[f],l=0;l<k.joints.length;++l)p.nodes[k.joints[l]]._is_joint=!0;k=null;1<g.length&&(k=new RD.SceneNode,k.name="root");for(f=0;f<g.length;++f){var e=l=g[f];null!=l.node&&(e=l.node);l=RD.GLTF.parseNode(null,
e,p);k||(k=l);1<g.length&&k.addChild(l);l.id=p.url.replace(/\//gi,"_")+"::node_"+f;m[l.id]=m[f]=l}if(p.animations&&p.animations.length){if(RD.Animation)for(k.animations=[],f=0;f<p.animations.length;++f)g=this.parseAnimation(f,p,m),g.id=p.filename+"::"+g.name,g&&(RD.Animations[g.id]=g,k.animations.push(g));else console.error("you must include rendeer-animation.js to allow animations");this.convert_skeletons&&this.convertSkinToSkeleton(k,k)}k.materials=this.gltf_materials;return k},parseNode:function(p,
a,k){var m=k.nodes[a];p=p||new RD.SceneNode;for(var g in m){var f=m[g];switch(g){case "name":p.name=f;break;case "translation":p.position=f;break;case "rotation":p.rotation=f;break;case "scale":p.scaling=f;var l=0;0>p.scaling[0]&&l++;0>p.scaling[1]&&l++;0>p.scaling[2]&&l++;1==l%2&&(p.flags.frontFace=GL.CW);break;case "matrix":p.fromMatrix(f);0>mat4.determinant(f)&&(p.flags.frontFace=GL.CW);break;case "mesh":if(f=RD.GLTF.parseMesh(f,k))for(p.mesh=f.name,p.primitives=[],l=0;l<f.info.groups.length;++l){var e=
f.info.groups[l],t=null;null!=e.material&&(t=this.parseMaterial(e.material,k));p.primitives.push({index:l,material:t?t.name:null,mode:e.mode})}break;case "skin":p.skin=this.parseSkin(f,k);break;case "children":if(f.length)for(l=0;l<f.length;++l)e=RD.GLTF.parseNode(null,f[l],k),p.addChild(e);break;case "extras":break;default:"_"!=g[0]&&console.log("gltf node info ignored:",g,m[g])}}if(p.mesh&&p.skin)for(f=gl.meshes[p.mesh],f.bones=[],l=0;l<p.skin.joints.length;++l)f.bones.push([p.skin.joints[l],p.skin.bindMatrices[l]]);
m.name||(m.name=p.name="node_"+a);m._is_joint&&(p.is_joint=!0);return p},parseMesh:function(p,a){for(var k=a.meshes[p],m=gl.meshes,g=[],f=[],l=0,e=0;e<k.primitives.length;++e){var t=this.parsePrimitive(k,e,a);if(t){t.start=l;l+=t.length;f.push(t);var h={vertexBuffers:{},indexBuffers:{}},c;for(c in t.buffers){var n=c;"bones"==n&&(n="bone_indices");"indices"==n||"triangles"==n?h.indexBuffers[n]={data:t.buffers[c]}:h.vertexBuffers[n]={data:t.buffers[c]}}g.push({mesh:h})}}l=null;1<g.length?l=GL.Mesh.mergeMeshes(g):
1==g.length&&(e=g[0].mesh,l=new GL.Mesh(e.vertexBuffers,e.indexBuffers),l.info&&e.info&&(l.info=e.info));if(!l)return null;for(e=0;e<k.primitives.length;++e)(g=l.info.groups[e])||(l.info.groups[e]=g={}),t=k.primitives[e],g.material=t.material,g.mode=null!=t.mode?t.mode:4,g.start=f[e].start,g.length=f[e].length;l.name=k.name;if(!l.name||this.rename_assets)l.name=a.filename+"::mesh_"+(k.name||p);l.updateBoundingBox();l.computeGroupsBoundingBoxes();return m[l.name]=l},parsePrimitive:function(p,a,k){var m=
{buffers:{}},g=m.buffers;p=p.primitives[a];if(p.extensions)if(p.extensions.KHR_draco_mesh_compression){if("undefined"==typeof DracoDecoderModule)throw"mesh data is compressed using Draco, draco_decoder.js not installed.";g=m.buffers=this.decompressDraco(p,k)}else throw"mesh data is compressed, this importer does not support it yet";else{null==!p.attributes.POSITION&&console.warn("gltf mesh without positions");for(var f in this.buffer_names){a=this.buffer_names[f];var l=this.flip_uv&&("coords"==a||
"coords1"==a),e=p.attributes[f];null!=e&&(l=this.parseAccessor(e,k,l))&&(g[a]=l)}null!=p.indices&&(g.triangles=this.parseAccessor(p.indices,k))}if(!g.vertices)return console.error("primitive without vertices"),null;m.mode=p.mode;m.material=p.material;m.start=0;m.length=g.triangles?g.triangles.length:g.vertices.length/3;return m},convertSkinToSkeleton:function(p,a){if(p.skin){var k=a.findNodeByName(p.skin.skeleton_root);p.skeleton||(p.skeleton=new RD.Skeleton);p.skeleton.importSkeleton(k||a)}for(k=
...
decompressDraco:function(p,a){this.draco_decoder||(this.draco_decoder=new this.decoderModule.Decoder);return this.decodePrimitive(this.draco_decoder,p,a)},decodePrimitive:function(p,a,k){var m={};a=k.buffers[k.bufferViews[a.extensions.KHR_draco_mesh_compression.bufferView].buffer];var g=a.dataview.buffer;k=this.decoderModule;a=new k.DecoderBuffer;a.Init(new Int8Array(g),g.byteLength);if(p.GetEncodedGeometryType(a)==k.TRIANGULAR_MESH){var f=new k.Mesh,g=p.DecodeBufferToMesh(a,f);if(!g.ok()||0===f.ptr)throw Error("GLTF Draco: Decoding failed: "+
g.error_msg());f.num_points();for(var l in this.buffer_names){var g=this.buffer_names[l],e=l;"COLOR_0"==e?e="COLOR":"TEXCOORD_0"==e&&(e="TEX_COORD");if(e=this.decodeBuffer(f,k[e],"coords"==g||"coords1"==g,p))m[g]=e.data}l=3*f.num_faces();g=4*l;e=k._malloc(g);p.GetTrianglesUInt32Array(f,g,e);m.triangles=(new Uint32Array(k.HEAPF32.buffer,e,l)).slice();k._free(e)}k.destroy(a);k.destroy(f);return m},decodeBuffer:function(p,a,k,m){if(null==a)return null;var g=this.decoderModule;a=m.GetAttributeId(p,a);
if(-1==a)return null;var f=m.GetAttribute(p,a);a=f.data_type();var l=f.num_components(),e=p.num_points();f.size();var t=e*l,h=this.draco_data_types[a],g=new g.DracoFloat32Array;m.GetAttributeFloatForAllPoints(p,f,g);p=new h(t);for(m=0;m<p.length;++m)p[m]=g.GetValue(m);if(k)for(m=1;m<p.length;m+=l)p[m]=1-p[m];return{num_points:e,num_comps:l,data_type:a,data:p}},parseAccessor:function(p,a,k,m,g){var f=a.accessors[p];if(!f)return console.warn("gltf accessor not found"),null;p=this.numComponents[f.type];
if(!p)return console.warn("gltf accessor of unknown type:",f.type),null;var l=f.count*p;g=null;switch(f.componentType){case RD.GLTF.FLOAT:g=new Float32Array(l);break;case RD.GLTF.UNSIGNED_INT:g=new Uint32Array(l);break;case RD.GLTF.SHORT:g=new Int16Array(l);break;case RD.GLTF.UNSIGNED_SHORT:g=new Uint16Array(l);break;case RD.GLTF.BYTE:g=new Int8Array(l);break;case RD.GLTF.UNSIGNED_BYTE:g=new Uint8Array(l);break;default:console.warn("gltf accessor of unsupported type: ",f.componentType),g=new Float32Array(l)}null==
m&&(m=a.bufferViews[f.bufferView]);if(!m)return console.warn("gltf bufferView not found"),null;l=a.buffers[m.buffer];if(!l||!l.data)return console.warn("gltf buffer not found or data not loaded"),null;if(m.byteStride&&m.byteStride!=p*g.BYTES_PER_ELEMENT)return console.warn("gltf buffer data is not tightly packed, not supported"),null;a=new Uint8Array(g.buffer);null==m.byteOffset&&(m.byteOffset=0);m=m.byteOffset+(f.byteOffset||0);m=l.dataview.subarray(m,m+a.length);a.set(m);if(k)for(k=1;k<g.length;k+=
...
a));k.normalTexture&&(g.textures.normal=this.parseTexture(k.normalTexture,a));k.emissiveTexture&&(g.textures.emissive=this.parseTexture(k.emissiveTexture,a));k.emissiveFactor&&(g.emissive=k.emissiveFactor);RD.Materials[g.name]=g;return this.gltf_materials[g.name]=g},parseTexture:function(p,a){var k=a.textures[p.index];if(!k)return console.warn("gltf texture not found"),null;var m=a.images[k.source],g="",f=null;m.uri?(f=m.uri,f.split(".").pop()):(f=a.url.replace(/[\/\.\:]/gi,"_")+"_image_"+p.index,
g=m.mimeType?m.mimeType.split("/").pop():"png",f+="."+g);var l={};if(!gl.textures[f])if(m.uri){var e=m.uri;if("data:"==e.substr(0,5)){var t=m.uri.indexOf(","),h=m.uri.substr(5,t),g=h.split("/").pop().toLowerCase(),f=a.folder+"/"+e+"image_"+p.index+"."+g,g=_base64ToArrayBuffer(m.uri.substr(t+1)),m=URL.createObjectURL(new Blob([g],{type:h})),m=GL.Texture.fromURL(m,this.texture_options);m.name=f;gl.textures[f]=m}else"blob:"==e.substr(0,5)?l.texture=e:l.texture=a.folder+"/"+e}else null!=m.bufferView&&
(h=a.bufferViews[m.bufferView],null==h.byteOffset&&(h.byteOffset=0),g=a.buffers[h.buffer].data.slice(h.byteOffset,h.byteOffset+h.byteLength),m=URL.createObjectURL(new Blob([g],{type:m.mimeType})),m=GL.Texture.fromURL(m,this.texture_options),m.name=f,gl.textures[f]=m);l.texture||(l.texture=f);null!=k.sampler&&(k=a.samplers[k.sampler],null!=k.magFilter&&(l.magFilter=k.magFilter),null!=k.minFilter&&(l.minFilter=k.minFilter));p.texCoord&&(l.uv_channel=p.texCoord);return l},parseSkin:function(p,a){var k=
a.skins[p],m={};null!=k.skeleton&&(m.skeleton_root=a.nodes[k.skeleton].name);var g=this.parseAccessor(k.inverseBindMatrices,a);if(!g)return console.warn("accessor is null"),null;m.bindMatrices=this.splitBuffer(g,16);m.joints=[];for(g=0;g<k.joints.length;++g){var f=a.nodes[k.joints[g]];m.joints.push(f.id||f.name)}return m},splitBuffer:function(p,a){p||console.warn("buffer is null");for(var k=p.length,m=[],g=0;g<k;g+=a)m.push(new p.constructor(p.subarray(g,g+a)));return m},parseAnimation:function(p,
a,k){k=a.animations[p];var m=new RD.Animation;m.name=k.name||"anim_"+p;for(var g=p=0;g<k.channels.length;++g){var f=new RD.Animation.Track,l=k.channels[g],e=k.samplers[l.sampler];f.target_node=a.nodes[l.target.node].name;f.target_property=l.target.path.toLowerCase();if(l=this.rename_animation_properties[f.target_property])f.target_property=l;var l=this.parseAccessor(e.input,a),t=this.parseAccessor(e.output,a),h=a.accessors[e.output].type,e=RD.TYPES[h];e==RD.VEC4&&"rotation"==f.target_property&&(e=
RD.QUAT);f.type=e;if(e=RD.TYPES_SIZE[e]){for(var h=t.length/e,c=new Float32Array((1+e)*h),n=0;n<h;++n){c[n*(1+e)]=l[n];var b=t.subarray(n*e,n*e+e);c.set(b,n*(1+e)+1)}f.data=c;f.packed_data=!0;p=Math.max(p,l[l.length-1]);m.addTrack(f)}else console.warn("gltf unknown type:",h)}m.duration=p;return m},loadFromFiles:function(p,a){function k(c){var b=c.target.result,h=this.extension;if("gltf"==h)b=JSON.parse(b),m.main=this.filename;else if("glb"==h)m.main=this.filename;else if("bin"==h)l.push(this.filename);
else if("jpeg"==h||"jpg"==h||"png"==h)c=URL.createObjectURL(new Blob([b],{type:c.target.mimeType})),h=GL.Texture.fromURL(c,{wrap:gl.REPEAT,extension:h}),h.name=this.filename,gl.textures[h.name]=h,gl.textures["/textures/"+h.name]&&(gl.textures["/textures/"+h.name]=h);m[this.filename]={filename:this.filename,data:b,extension:this.extension};g--;0==g&&(m.binaries=l,f.load(m,function(b){a&&a(b)}))}for(var m={},g=p.length,f=this,l=[],e=0;e<p.length;++e){var t=p[e],h=new FileReader,c=t.name.split("."),
c=c[c.length-1].toLowerCase();h.onload=k;h.filename=t.name;h.extension=c;"gltf"==c?h.readAsText(t):h.readAsArrayBuffer(t)}},removeRootPathFromTextures:function(p,a){if(a)for(var k in p){var m=p[k],g;for(g in m.textures){var f=m.textures[g];f&&(f.constructor===String&&0==f.indexOf(ROOM.root_path)&&-1!=f.texture.indexOf("/")?f.substr(ROOM.root_path.length):f.texture&&0==f.texture.indexOf(ROOM.root_path)&&-1!=f.texture.indexOf("/")&&(f.texture=f.texture.substr(ROOM.root_path.length)))}}}};
RD.SceneNode.prototype.loadGLTF=function(p,a){function k(f){m.loading=!1;f&&m.addChild(f);a&&a(m,f)}var m=this;if(RD.GLTF.prefabs[p]){var g=new RD.SceneNode;g.configure(RD.GLTF.prefabs[p]);k(g)}else this.loading=!0,RD.GLTF.load(p,k)};function _base64ToArrayBuffer(p){p=window.atob(p);for(var a=p.length,k=new Uint8Array(a),m=0;m<a;m++)k[m]=p.charCodeAt(m);return k.buffer}"undefined"!=typeof DracoDecoderModule&&RD.GLTF.installDracoModule(RD.GLTF.onReady);
(function(p){function a(f){this.renderer=f;this.mode=a.FORWARD;this.visible_layers=65535;this.bgcolor=vec4.fromValues(0.1,0.1,0.1,1);this.environment_texture=null;this.render_skybox=!0;this.environment_sh_coeffs=this.skybox_texture=null;this.environment_rotation=180;this.emissive_factor=this.occlusion_factor=this.exposure=this.environment_factor=1;this.postfx_shader_name=null;this.allow_overlay=this.timer_queries_enabled=!0;this.brightness=this.contrast=1;this.gamma=2.2;this.parallax_reflection=!1;
...
this._instancing=this.skin=null;this._render_priority=0}var m=p.RD;a.FORWARD=1;a.DEFERRED=2;a.MACROS={UVS2:1,COLOR:2,POINTS:4,INSTANCING:8,SKINNING:16,PARALLAX_REFLECTION:32};a.maps="albedo metallicRoughness occlusion normal emissive opacity displacement detail".split(" ");a.maps_sampler=[];for(p=0;p<a.maps.length;++p)a.maps_sampler[p]="u_"+a.maps[p]+"_texture";a.prototype.render=function(f,g,e,k,h,c){this.renderer=f;this.current_camera=e;this.mode==a.FORWARD?this.renderForward(g,e,h,c):this.mode==
a.DEFERRED&&this.renderDeferred(g,e,c)};a.prototype.fillGlobalUniforms=function(a){var g=this.getBRDFIntegratorTexture();g&&g.bind(0);this.environment_texture?(this.environment_texture.bind(1),this.global_uniforms.u_use_environment_texture=!0):this.global_uniforms.u_use_environment_texture=!1;this.environment_sh_coeffs?(this.global_uniforms.u_useDiffuseSH=!0,this.global_uniforms.u_sh_coeffs=this.environment_sh_coeffs):this.global_uniforms.u_useDiffuseSH=!1;this.global_uniforms.u_skybox_info[0]=this.environment_rotation*
...
this.gamma:1)};a.prototype.renderForward=function(a,g,e,k){var h=Math.floor(gl.viewport_data[2]*this.resolution_factor),c=Math.floor(gl.viewport_data[3]*this.resolution_factor),h=Math.min(h,this.max_texture_size),c=Math.min(c,this.max_texture_size);this.use_rendertexture&&!e&&(this.frame_texture&&this.frame_texture.width==h&&this.frame_texture.height==c||(this.frame_texture=new GL.Texture(h,c,{format:gl.RGBA,type:gl.HIGH_PRECISION_FORMAT}),this.final_fbo?this.final_fbo.setTextures([this.frame_texture]):
...
this.renderer,g);LEvent.trigger(this,"renderGizmos",this);this.use_rendertexture&&!e&&this.renderFinalBuffer();if(this.overlay_rcs.length)for(a=this.overlay_rcs,this.global_uniforms.u_gamma=1,this.global_uniforms.u_exposure=1,gl.clear(gl.DEPTH_BUFFER_BIT),g=0;g<a.length;++g)e=a[g],this.renderMeshWithMaterial(e.model,e.mesh,e.material,e.index_buffer_name,e.group_index,e.node.extra_uniforms,e.reverse_faces,e.skin)};a.prototype.renderNodes=function(f,g,e){this.resetRenderCallsPool();for(var k=this.render_calls.length=
0;k<f.length;++k)this.getNodeRenderCalls(f[k],g,e);f=this.render_calls;if(this.onFilterRenderCalls)this.onFilterRenderCalls(f);for(k=0;k<f.length;++k)f[k].computeRenderPriority(g._position);f=f.sort(a.rc_sort_function);this.allow_instancing&&gl.extensions.ANGLE_instanced_arrays&&(f=this.groupRenderCallsForInstancing(f));g=(this.alpha_composite_callback||this.alpha_composite_target_texture)&&GL.FBO.current;e=!0;for(var h=this.overlay_rcs,k=h.length=0;k<f.length;++k){var c=f[k];if(c.material.overlay&&
this.allow_overlay)h.push(c);else{e&&g&&"BLEND"==c.material.alphaMode&&(e=!1,this.alpha_composite_target_texture&&GL.FBO.current.color_textures[0].copyTo(this.alpha_composite_target_texture),this.alpha_composite_callback&&this.alpha_composite_callback(GL.FBO.current,this.alpha_composite_target_texture));var n=c.model;c._instancing&&c._instancing.length&&(n=GL.linearizeArray(c._instancing,Float32Array));this.renderMeshWithMaterial(n,c.mesh,c.material,c.index_buffer_name,c.group_index,c.node.extra_uniforms,
...
this.brightness;this.fx_uniforms.u_gamma=this.gamma;this.frame_texture.copyTo(this.final_texture,gl.shaders.fxaa_tonemapper,this.fx_uniforms);var a=null;this.postfx_shader_name&&(a=gl.shaders[this.postfx_shader_name])&&a.setUniform("u_size",[this.final_texture.width,this.final_texture.height]);this.final_texture.toViewport(a)};a.prototype.getNodeRenderCalls=function(f,g,e){var k=null;if(f._mesh)k=f._mesh;else if(f.mesh&&(k=gl.meshes[f.mesh],!k))return;void 0===e&&(e=65535);f.updateGlobalMatrix(!0);
if(k&&!1!==f.flags.visible&&f.layers&e){e=f.skeleton||f.skin||null;if(this.test_visibility&&!e){a.temp_bbox||(a.temp_bbox=BBox.create(),a.aabb_center=BBox.getCenter(a.temp_bbox),a.aabb_halfsize=BBox.getHalfsize(a.temp_bbox));BBox.transformMat4(a.temp_bbox,k.getBoundingBox(),f._global_matrix);if(g.testBox(a.aabb_center,a.aabb_halfsize)==m.CLIP_OUTSIDE)return;f._last_rendered_frame=this.renderer.frame}if(f.primitives&&f.primitives.length)for(g=0;g<f.primitives.length;++g){var h=f.primitives[g],c=null;
if(c=h.material?m.Materials[h.material]:this.default_material)h=this.getRenderCallFromPool(),h.material=c,h.model=f._global_matrix,h.mesh=k,h.group_index=g,h.node=f,h._render_priority=c.render_priority||0,h._instancing=null,h.reverse_faces=f.flags.frontFace==GL.CW,h.skin=e,this.render_calls.push(h)}else f.material&&(c=m.Materials[f.material])&&(h=this.getRenderCallFromPool(),h.material=c,h.model=f._global_matrix,h.mesh=k,h.group_index=-1,h.node=f,h._instancing=null,h.skin=e,h._render_priority=c.render_priority||
0,h.reverse_faces=f.flags.frontFace==GL.CW,this.render_calls.push(h))}};a.prototype.groupRenderCallsForInstancing=function(a){for(var g={},e=0,k=0;k<a.length;++k){var h=a[k],c=null,c=h._instancing||h.skin?e++:h.mesh.name+":"+h.group_index+"/"+h.material.name+(h.reverse_faces?"[R]":"");g[c]?g[c].push(h):g[c]=[h]}a=[];for(k in g)if(e=g[k],0!=e.length)if(1==e.length)h=e[0],this.debug_instancing||a.push(h);else{h=this.getRenderCallFromPool();h.copyFrom(e[0]);h._instancing=Array(e.length);for(c=0;c<e.length;++c)h._instancing[c]=
e[c].model;a.push(h)}return a};a.rc_sort_function=function(a,g){return g._render_priority-a._render_priority};a.prototype.setParallaxReflectionTransform=function(a){this.parallax_reflection_matrix.set(a);this.parallax_reflection=!0;this.global_uniforms.u_cube_reflection_matrix=this.parallax_reflection_matrix;mat4.invert(this.parallax_reflection_matrix_inv,this.parallax_reflection_matrix);this.global_uniforms.u_inv_cube_reflection_matrix=this.parallax_reflection_matrix_inv};a.prototype.resetShadersCache=
function(){this.compiled_shaders={}};a.prototype.getShader=function(f,g,e){e=e||"";var k=e+":"+g,h=this.compiled_shaders[k];h||(h=this.compiled_shaders[k]=new Map);if(k=h.get(f))return k;if(!this.renderer.shader_files)return null;e=this.renderer.shader_files[e||"default.vs"];g=this.renderer.shader_files[g];if(!e||!g)return null;k=null;if(f){var k={},c;for(c in a.MACROS)f&a.MACROS[c]&&(k[c]="")}k=new GL.Shader(e,g,k);h.set(f,k);return k};a.prototype.renderRenderCall=function(a){var g=a.model;a._instancing&&
a._instancing.length&&(g=new Float32Array(a._instancing.flat()));this.renderMeshWithMaterial(g,a.mesh,a.material,a.index_buffer_name,a.group_index,a.node.extra_uniforms,a.reverse_faces,a.skin)};a.default_backface_color=[0.1,0.1,0.1];a.prototype.renderMeshWithMaterial=function(f,g,e,k,h,c,n,b){var r=this.renderer,s=null;if(!e||e.constructor===String)throw"no material in renderMeshWithMaterial";if(!("BLEND"==e.alphaMode&&0>=e.color[3])){var q=this.material_uniforms,p=this.sampler_uniforms,v=f.length/
16;q.u_albedo=e.color.subarray(0,3);q.u_emissive.set(e.emissive||m.ZERO);q.u_emissive[3]=e.emissive_clamp_to_edge?1:0;1!=this.emissive_factor&&vec3.scale(q.u_emissive,q.u_emissive,this.emissive_factor);q.u_backface_color=e.backface_color||a.default_backface_color;s=0;g.vertexBuffers.coords1&&(s|=a.MACROS.UVS2);g.vertexBuffers.colors&&(s|=a.MACROS.COLOR);b&&(s|=a.MACROS.SKINNING);this.parallax_reflection&&(s|=a.MACROS.PARALLAX_REFLECTION);e.primitive==GL.POINTS&&(s|=a.MACROS.POINTS);1<v&&(s|=a.MACROS.INSTANCING);
this.overwrite_shader_name?s=gl.shaders[this.overwrite_shader_name]:this.overwrite_shader_mode?s=this.getShader(s,this.overwrite_shader_mode):e.shader_name?s=gl.shaders[e.shader_name]:e.overlay?s=this.getShader(s,"overlay.fs"):"pbrMetallicRoughness"==e.model&&this.quality?(q.u_metalness=e.metallicFactor,q.u_roughness=e.roughnessFactor,q.u_metallicRough=Boolean(e.textures.metallicRoughness),s=this.getShader(s,"pbr.fs")):s=this.getShader(s,"nopbr.fs");if(s){q.u_alpha=e.opacity;q.u_alpha_cutoff=0;q.u_normalFactor=
null!=e.normalmapFactor?e.normalmapFactor:1;q.u_displacement_factor=null!=e.displacementFactor?e.displacementFactor:1;e.uv_transform?this.texture_matrix.set(e.uv_transform):mat3.identity(this.texture_matrix);for(var z=2,u=q.u_maps_info,x=0;x<a.maps.length;++x){var y=a.maps[x];u[x]=-1;if(y=e.textures[y]){var B=null;y.constructor===Object?B=y.texture:y.constructor===String&&(B=y,y=null);if(B){var w=a.maps_sampler[x];if(!s||s.samplers[w]){var D=gl.textures[B];D||(r.autoload_assets&&-1!=B.indexOf(".")&&
r.loadTexture(B,r.default_texture_settings),D=gl.textures.white);B=16>this.max_textures?z++:x+2;p[w]=D.bind(B);u[x]=y&&null!=y.uv_channel?Math.clamp(y.uv_channel,0,3):0}}}}n||gl.frontFace(GL.CCW);r.enableItemFlags(e);n&&gl.frontFace(GL.CW);"BLEND"==e.alphaMode?(gl.enable(gl.BLEND),e.additive||"ADD"==e.blendMode?gl.blendFunc(gl.SRC_ALPHA,gl.ONE):"MULTIPLY"==e.blendMode?gl.blendFunc(gl.DST_COLOR,gl.ONE_MINUS_SRC_ALPHA):gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA),gl.depthMask(!1),q.u_alpha_cutoff=
0):"MASK"==e.alphaMode?q.u_alpha_cutoff=e.alphaCutoff:gl.disable(gl.BLEND);if(b)if(b.constructor===m.Skeleton)s.setUniform("u_bones",b.computeFinalBoneMatrices(),g);else if(b._bone_matrices)s.setUniform("u_bones",b._bone_matrices);else return;1==v&&r._uniforms.u_model.set(f);b&&mat4.identity(r._uniforms.u_model);s.uniforms(r._uniforms);s.uniforms(this.global_uniforms);s.uniforms(e.uniforms);s.uniforms(q);s.uniforms(p);c&&s.uniforms(c);e.primitive==GL.POINTS&&s.setUniform("u_pointSize",e.point_size||
-1);c=null;null!=h&&g.info&&g.info.groups&&g.info.groups[h]&&(c=g.info.groups[h]);h=this._instancing_uniforms;h.u_model=f;e.flags.preAlpha&&(gl.colorMask(!1,!1,!1,!1),gl.depthMask(!0),1<v?s.drawInstanced(g,void 0===e.primitive?gl.TRIANGLES:e.primitive,k,h):c?s.drawRange(g,e.primitive,c.start,c.length,k):s.draw(g,e.primitive,k),gl.colorMask(!0,!0,!0,!0),gl.depthFunc(gl.LEQUAL),this.rendered_render_calls++);1<v?c?s.drawInstanced(g,void 0===e.primitive?gl.TRIANGLES:e.primitive,k,h,c.start,c.length):
s.drawInstanced(g,void 0===e.primitive?gl.TRIANGLES:e.primitive,k,h):c?s.drawRange(g,e.primitive,c.start,c.length,k):s.draw(g,e.primitive,k);this.rendered_render_calls++;r.disableItemFlags(e);n&&gl.frontFace(GL.CCW);gl.depthFunc(gl.LESS);gl.depthMask(!0)}}};a.prototype.renderDeferred=function(a,g){};a.prototype.prepareBuffers=function(a){};a.prototype.renderToGBuffers=function(a,g){};a.prototype.renderFinalPass=function(a,g){};a.prototype.applyPostFX=function(){};a.prototype.getRenderCallFromPool=
function(){if(this.used_render_calls<this.render_calls_pool.length){var a=this.render_calls_pool[this.used_render_calls];this.used_render_calls++;return a}a=new m.RenderCall;a.id=this.used_render_calls;this.render_calls_pool.push(a);this.used_render_calls++;return a};a.prototype.resetRenderCallsPool=function(){this.used_render_calls=0};a.prototype.renderSkybox=function(a){if((!this.onRenderSkybox||!this.onRenderSkybox(this,a))&&this.environment_texture&&this.render_skybox){var g=gl.meshes.cube,e=
gl.shaders[this.overwrite_shader_name||"skybox"];if(e){var k=this.skybox_texture||this.environment_texture;if(k&&k.texture_type===GL.TEXTURE_CUBE_MAP){gl.disable(gl.CULL_FACE);gl.disable(gl.DEPTH_TEST);gl.disable(gl.BLEND);var h=this.renderer._model_matrix;mat4.identity(h);mat4.translate(h,h,a.position);mat4.scale(h,h,[10,10,10]);this.renderer.setModelMatrix(h);e.uniforms(this.renderer._uniforms);e.uniforms({u_color_texture:k.bind(1),u_is_rgbe:!1,u_exposure:this.exposure,u_mipmap_offset:0,u_rotation:this.environment_rotation*
DEG2RAD,u_camera_position:a.position});e.draw(g,GL.TRIANGLES)}}}};a.prototype.loadEnvironment=function(a,g,e){var k=this,h=gl.textures[a];h?(e?k.skybox_texture=h:(h.shs&&(k.environment_sh_coeffs=h.shs),k.environment_texture=h),g&&g(h)):HDRE.load(a,function(c){var h=c.toTexture(!0);h&&(gl.textures[a]=h,e?k.skybox_texture=h:(h.shs=c.shs?c.shs:null,k.environment_texture=h,h.shs&&(k.environment_sh_coeffs=h.shs)));g&&g(h)})};a.prototype.captureEnvironment=function(a,g,e){e=e||256;this.capture_environment_texture||
(this.capture_environment_texture=new GL.Texture(e,e,{format:gl.RGBA,type:GL.UNSIGNED_BYTE,minFilter:gl.LINEAR_MIPMAP_LINEAR,texture_type:GL.TEXTURE_CUBE_MAP}));var k=this.use_rendertexture;this.use_rendertexture=!1;this.renderer.renderToCubemap(this.capture_environment_texture,a,g);this.use_rendertexture=k;this.capture_environment_texture.bind(0);gl.generateMipmap(this.capture_environment_texture.texture_type);this.environment_texture||(this.environment_texture=new GL.Texture(e,e,{format:gl.RGBA,
type:GL.UNSIGNED_BYTE,minFilter:gl.LINEAR_MIPMAP_LINEAR,texture_type:GL.TEXTURE_CUBE_MAP}));this.capture_environment_texture.copyTo(this.environment_texture)};a.prototype.getBRDFIntegratorTexture=function(a){if(gl.textures.brdf_integrator)return gl.textures.brdf_integrator;var g=gl.shaders.brdf_integrator;if(g){var e=gl.textures.brdf_integrator=new GL.Texture(128,128,{type:gl.FLOAT,texture_type:gl.TEXTURE_2D,filter:gl.LINEAR});if(a)return fetch(a).then(function(a){return a.arrayBuffer()}).then(function(a){e.uploadData(new Float32Array(a),
{no_flip:!0})}),e;var k=gl.textures.hammersley_sample_texture;k||(k=this.createHammersleySampleTexture());e.drawTo(function(a){k&&k.bind(0);g.uniforms({u_hammersley_sample_texture:0}).draw(GL.Mesh.getScreenQuad(),gl.TRIANGLES)});return e}};a.prototype.createHammersleySampleTexture=function(a){a=a||8192;for(var g=3*a,e=new Float32Array(g),k=0;k<g;k+=3){var h=2*Math.radical_inverse(k)*Math.PI;e[k]=Math.cos(h);e[k+1]=Math.sin(h);e[k+2]=0}a=new GL.Texture(a,1,{pixel_data:e,type:GL.FLOAT,format:GL.RGB});
return gl.textures.hammersley_sample_texture=a};a.prototype.setClippingPlane=function(a,g){a?this.global_uniforms.u_clipping_plane.set([g[0],g[1],g[2],vec3.dot(a,g)]):this.global_uniforms.u_clipping_plane.set([0,0,0,0])};a.prototype.startGPUQuery=function(){if(gl.extensions.EXT_disjoint_timer_query&&this.timer_queries_enabled){var a=gl.extensions.EXT_disjoint_timer_query;this._waiting_gpu_query||(void 0===this._useQueryTimestamps&&(this._useQueryTimestamps=!1,0<a.getQueryEXT(a.TIMESTAMP_EXT,a.QUERY_COUNTER_BITS_EXT)&&
(this._useQueryTimestamps=!0)),gl.getParameter(a.GPU_DISJOINT_EXT),this._useQueryTimestamps?(this._start_gpu_query=a.createQueryEXT(),this._end_gpu_query=a.createQueryEXT(),a.queryCounterEXT(this._start_gpu_query,a.TIMESTAMP_EXT)):(this._timeElapsed_gpu_query=a.createQueryEXT(),a.beginQueryEXT(a.TIME_ELAPSED_EXT,this._timeElapsed_gpu_query)))}};a.prototype.endGPUQuery=function(){if(gl.extensions.EXT_disjoint_timer_query&&this.timer_queries_enabled&&!this._waiting_gpu_query){var a=gl.extensions.EXT_disjoint_timer_query;
this._useQueryTimestamps?a.queryCounterEXT(this._end_gpu_query,a.TIMESTAMP_EXT):a.endQueryEXT(a.TIME_ELAPSED_EXT);this._waiting_gpu_query=!0}};a.prototype.resolveQueries=function(){if(gl.extensions.EXT_disjoint_timer_query&&this.timer_queries_enabled){var a=gl.extensions.EXT_disjoint_timer_query,g=this._start_gpu_query,e=this._end_gpu_query,k=this._timeElapsed_gpu_query,h=this._useQueryTimestamps;if(g||e||k){var c=gl.getParameter(a.GPU_DISJOINT_EXT),n;if(!c&&(n=h?a.getQueryObjectEXT(e,a.QUERY_RESULT_AVAILABLE_EXT):
a.getQueryObjectEXT(k,a.QUERY_RESULT_AVAILABLE_EXT))){var b;h?(b=a.getQueryObjectEXT(g,a.QUERY_RESULT_EXT),b=a.getQueryObjectEXT(e,a.QUERY_RESULT_EXT)-b):b=a.getQueryObjectEXT(k,a.QUERY_RESULT_EXT);this.frame_time=1E-6*b}if(n||c)h?(a.deleteQueryEXT(g),a.deleteQueryEXT(e),this._end_gpu_query=this._start_gpu_query=null):(a.deleteQueryEXT(k),this._timeElapsed_gpu_query=null),this._waiting_gpu_query=!1}return this.frame_time}};Math.radical_inverse=function(a){for(var g=0,e=0.5;a;e*=0.5,a>>=1)a&1&&(g+=
e);return g};var g=vec3.create();k.prototype.copyFrom=function(a){this.name=a.name;this.id=a.id;this.mesh=a.mesh;this.model=a.model;this.index_buffer_name=a.index_buffer_name;this.group_index=a.group_index;this.material=a.material;this.reverse_faces=a.reverse_faces;this.node=a.node;this.skin=a.skin;this._instancing=a._instancing;this._render_priority=a._render_priority};k.prototype.computeRenderPriority=function(a){this.name=this.node.name;var k=this.mesh.getBoundingBox();k&&(k=mat4.multiplyVec3(g,
this.model,k),this._render_priority=this.material.render_priority||0,a=vec3.distance(a,k),"BLEND"==this.material.alphaMode?(this._render_priority+=0.001*a,this._render_priority-=100):this._render_priority+=1E3-0.001*a)};m.PBRPipeline=a;m.RenderCall=k})(this);
...
!0;this.shadowmap.fbo.bind();gl.clear(gl.DEPTH_BUFFER_BIT);a.shader_overwrite="flat";if(m.constructor===Array)for(var f=0;f<m.length;++f)a.render(m[f],this.camera,null,g||255);else a.render(m,this.camera,null,g||255);a.shader_overwrite=null;this.shadowmap.fbo.unbind();a.generating_shadowmap=!1;this.shadowmap.texture.bind(4);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR)}};RD.Light=a})("undefined"!=typeof window?window:"undefined"!=typeof self?self:global);
(function(p){function a(){this.name="";this.tracks=[];this.duration=10}function k(){this.enabled=!0;this.target_property=this.target_node="";this.type=e.SCALAR;this.data=[];this.packed_data=!1;this._target=null}function m(a,c,e){return a*(1-e)+c*e}function g(){this.bones=[];this.global_bone_matrices=[];this.bones_by_name=new Map}function f(){this.name="";this.model=mat4.create();this.parent=-1;this.num_children=this.layer=0;this.index=-1;this.children=new Int8Array(16)}function l(){this.skeleton=
new g;this.duration=0;this.samples_per_second=30;this.num_keyframes=this.num_animated_bones=0;this.bones_map=new Uint8Array(64);this.keyframes=null}var e=p.RD=p.RD||{};e.Animations={};a.prototype.addTrack=function(a,c){if(c)for(var e=0;e<this.tracks.length;++e)if(this.tracks[e].target_node==a.target_node){this.tracks.splice(e+1,0,a);return}this.tracks.push(a)};a.prototype.applyAnimation=function(a,c,e){for(var b=0;b<this.tracks.length;++b){var f=this.tracks[b];!1!==f.enabled&&f.applyTrack(a,c,e)}};
a.prototype.serialize=function(){for(var a={name:this.name,duration:this.duration,tracks:[]},c=0;c<this.tracks.length;++c)a.tracks.push(this.tracks[c].serialize());return a};a.prototype.configure=function(a){this.name=a.name;this.duration=a.duration;for(var c=this.tracks.length=0;c<a.tracks.length;++c){var f=new e.Animation.Track;f.configure(a.tracks[c]);this.tracks.push(f)}return a};a.prototype.findNearestLeft=function(a){for(var c=0,e=0;e<this.tracks.length;++e){var b=this.tracks[e],f=b.findTimeIndex(a);
-1!=f&&(b=b.data[f],b>c&&(c=b))}return c};a.prototype.findNearestRight=function(a){for(var c=this.duration,e=0;e<this.tracks.length;++e){var b=this.tracks[e],f=b.findTimeIndex(a);-1!=f&&(b=b.data[f+1],null!=b&&b<c&&(c=b))}return c};e.Animation=a;a.Track=k;Object.defineProperty(k.prototype,"value_size",{set:function(a){throw"cannot be set, use type instead";},get:function(){return e.TYPES_SIZE[this.type]}});k.prototype.addKeyframe=function(a,c,e){1<this.value_size&&(c=new Float32Array(c));for(var b=
0;b<this.data.length;++b)if(!(this.data[b][0]<a))return this.data[b][0]!=a||e?this.data.splice(b,0,[a,c]):this.data[b][1]=c,b;this.data.push([a,c]);return this.data.length-1};k.prototype.getKeyframe=function(a){if(0>a||a>=this.data.length)return console.warn("keyframe index out of bounds"),null;var c=e.TYPES_SIZE[this.type];return this.packed_data?(a*=1+c,a>this.data.length-c?null:[this.data[a],this.data.subarray(a+1,a+c+1)]):this.data[a]};k.prototype.findTimeIndex=function(a){var c=this.data;if(!c||
0==c.length)return-1;var f=e.TYPES_SIZE[this.type];if(this.packed_data){var f=f+1,b=c.length/f,g=0,k=0,q=b;if(0==b)return-1;if(1==b)return 0;if(c[(q-1)*f]<a)return q-1;for(;q>=g;){k=0.5*(q+g)|0;b=c[k*f];if(b==a)break;if(g==q-1)return g;b<a?g=k:q=k}return k}b=c.length;k=g=0;q=b;if(0==b)return-1;if(1==b)return 0;if(c[q-1][0]<a)return q-1;for(;q>=g;){k=0.5*(q+g)|0;b=c[k][0];if(b==a)break;if(g==q-1)return g;b<a?g=k:q=k}return k};k.prototype.getSample=function(a,c,e){return this.data&&0!==this.data.length?
this.packed_data?this.getSamplePacked(a,c,e):this.getSampleUnpacked(a,c,e):void 0};k.prototype.getSampleUnpacked=function(a,c,f){var b=e.TYPES_SIZE[this.type];a=Math.clamp(a,0,this.data[this.data.length-1][0]);var g=this.findTimeIndex(a);-1===g&&(g=0);var k=g,q=g+1,l=this.data,b=e.TYPES_SIZE[this.type];if(!c||0==b||1==l.length||q==l.length||0==k&&this.data[0][0]>a)return this.data[g][1];k=l[k];q=l[q];a=(q[0]-a)/(q[0]-k[0]);1<b&&(f=f||this._result,f&&f.length==b||(f=this._result=new Float32Array(b)));
if(c===e.LINEAR)return 1==b?k[1]*a+q[1]*(1-a):e.interpolateLinear(k[1],q[1],a,f,this.type,b,this);if(c===e.CUBIC){c=0<g?l[g-1]:k;g=g<l.length-2?l[g+2]:q;if(1===b)return e.EvaluateHermiteSpline(k[1],q[1],c[1],g[1],1-a);f=e.EvaluateHermiteSplineVector(k[1],q[1],c[1],g[1],1-a,f);this.type==e.QUAT?(quat.slerp(f,q[1],k[1],a),quat.normalize(f,f)):this.type==e.TRANS10&&(b=f.subarray(3,7),k=k[1].subarray(3,7),q=q[1].subarray(3,7),quat.slerp(b,q,k,a),quat.normalize(b,b));return f}return null};k.prototype.getSamplePacked=
function(a,c,f){if(!this.data.length)return null;var b=e.TYPES_SIZE[this.type];a=Math.clamp(a,0,this.data[this.data.length-b-1]);var g=this.findTimeIndex(a);-1==g&&(g=0);var k=b+1,q=g,l=g+1,m=this.data,p=m.length/k;if(!c||1==p||l==p||0==q&&this.data[0]>a)return this.getKeyframe(g)[1];1<b&&(f=f||this._result,f&&f.length==b||(f=this._result=new Float32Array(b)));var t=m.subarray(q*k,(q+1)*k),q=m.subarray(l*k,(l+1)*k);a=(q[0]-a)/(q[0]-t[0]);if(c===e.LINEAR){if(1==b)return t[1]*a+q[1]*(1-a);k=t.subarray(1,
b+1);q=q.subarray(1,b+1);return e.interpolateLinear(k,q,a,f,this.type,b,this)}if(c===e.CUBIC){if(0===b)return t[1];c=0<g?m.subarray((g-1)*k,g*k):t;l=l<p-1?m.subarray((l+1)*k,(l+2)*k):q;if(1===b)return e.EvaluateHermiteSpline(t[1],q[1],c[1],l[1],1-a);b=t.subarray(1,k);q=q.subarray(1,k);f=e.EvaluateHermiteSplineVector(b,q,c.subarray(1,k),l.subarray(1,k),1-a,f);this.type==e.QUAT?(quat.slerp(f,q,b,a),quat.normalize(f,f)):this.type==e.TRANS10&&(k=f.subarray(3,7),b=b.subarray(3,7),q=q.subarray(3,7),quat.slerp(k,
q,b,a),quat.normalize(k,k));return f}return null};k.prototype.applyTrack=function(a,c,f){if(a)return c=this.getSample(c,f),a.constructor===e.SceneNode?(f=null,(f=a.name==this.target_node?a:a.findNodeByName(this.target_node))&&(f[this.target_property]=c)):a.constructor===e.Skeleton&&(a=a.getBone(this.target_node))&&this.type==e.MAT4&&a.model.set(c),c};k.prototype.serialize=function(){return{enabled:this.enabled,target_node:this.target_node,target_property:this.target_property,type:this.type,data:this.data.constructor==
Array?this.data.concat():typedArrayToArray(this.data),packed_data:this.packed_data}};k.prototype.configure=function(a){this.enabled=a.enabled;this.target_node=a.target_node;this.target_property=a.target_property;if(a.property){var c=a.property.indexOf("/");this.target_node=a.property.substr(0,c);this.target_property=a.property.substr(c+1)}null!=a.type&&(this.type=a.type.constructor===String?e.TYPES[a.type.toUpperCase()]||0:a.type);a.packed_data||a.data.constructor===Float32Array?(this.packed_data=
!0,this.data=new Float32Array(a.data)):(this.packed_data=!1,this.data=a.data.concat())};e.interpolateLinear=function(a,c,f,b,g,k,q){if(1==k)return a*f+c*(1-f);b=b||q._result;b&&b.length==k||(b=q._result=new Float32Array(k));switch(g){case e.QUAT:quat.slerp(b,c,a,f);quat.normalize(b,b);break;case e.TRANS10:for(g=0;3>g;g++)b[g]=a[g]*f+c[g]*(1-f);for(g=7;10>g;g++)b[g]=a[g]*f+c[g]*(1-f);a=a.subarray(3,7);c=c.subarray(3,7);k=b.subarray(3,7);quat.slerp(k,c,a,f);quat.normalize(k,k);break;default:for(g=0;g<
k;g++)b[g]=a[g]*f+c[g]*(1-f)}return b};e.EvaluateHermiteSpline=function(a,c,e,b,f){var g=f*f,k=g*f;return(2*k-3*g+1)*a+(-2*k+3*g)*c+(k-2*g+f)*(c-e)+(k-g)*(b-a)};e.EvaluateHermiteSplineVector=function(a,c,f,b,e,g){g=g||new Float32Array(g.length);var k=e*e,l=k*e,m=2*l-3*k+1,p=-2*l+3*k;e=l-2*k+e;for(var k=l-k,l=0,t=g.length;l<t;++l)g[l]=m*a[l]+p*c[l]+e*(c[l]-f[l])+k*(b[l]-a[l]);return g};Math.lerp||(Math.lerp=m);e.Skeleton=g;g.Bone=f;f.prototype.serialize=function(){return{name:this.name,model:typedArrayToArray(this.model),
parent:this.parent,layer:this.layer,children:this.num_children?typedArrayToArray(this.children.subarray(0,this.num_children)):null}};f.prototype.configure=function(a){this.name=a.name;this.model.set(a.model);this.parent=a.parent;this.layer=a.layer;this.num_children=0;this.index=null!=a.index?a.index:-1;a.children&&(this.children.set(a.children),this.num_children=a.children.constructor===Array?a.children.length:a.num_children)};f.prototype.copyFrom=f.prototype.configure;g.prototype.applyTransformToBones=
function(a,c){var e=this.getBone(a);e&&mat4.multiply(e.model,e.model,c)};g.prototype.getBone=function(a){return this.bones[this.bones_by_name.get(a)]};g.identity=mat4.create();g.prototype.getBoneMatrix=function(a,c){var e=-1;a.constructor===String?e=this.bones_by_name.get(a):a.constructor===Number&&(e=a);return void 0===e?g.identity:c?this.global_bone_matrices[e]:this.bones[e].model};g.prototype.importSkeleton=function(a,c){function e(a){var c=new f;c.name=a.name||a.id;a.model?c.model.set(a.model):
a.transform&&a.getLocalMatrix&&c.model.set(a.getLocalMatrix());c.index=g.length;g.push(c);b.bones_by_name.set(c.name,c.index);b.global_bone_matrices.push(mat4.create());if(a.children&&a.children.length){c.num_children=a.children.length;for(var h=0;h<a.children.length;++h){var k=e(a.children[h]);c.children[h]=k.index;k.parent=c.index}}return c}var b=this;g?this.bones.length=0:this.bones=[];var g=this.bones;e(a);c&&mat4.mul(g[0].model,c,g[0].model);this.updateGlobalMatrices()};g.temp_mat4=mat4.create();
...
mat4.multiply(g,g,c.bind_matrix);e&&mat4.multiply(g,e,g)}return a};g.prototype.updateGlobalMatrices=function(){var a=this.bones;if(a.length){var c=this.bones.length;this.global_bone_matrices[0].set(a[0].model);for(var e=1;e<c;++e){var b=a[e];mat4.multiply(this.global_bone_matrices[e],this.global_bone_matrices[b.parent],b.model)}}};g.prototype.assignLayer=function(a,c){};g.temp_vec3=vec3.create();g.temp_vec4=vec4.create();g.temp_mat4=mat4.create();g.prototype.applyTracksAnimation=function(a,c){for(var f=
g.temp_vec3,b=g.temp_vec4,k=g.temp_mat4,l=0;l<a.tracks.length;++l){var q=a.tracks[l],m=this.bones_by_name.get(q.target_node);null!=m&&(m=this.bones[m],"model"==q.target_property||"matrix"==q.target_property?q.getSample(c,e.LINEAR,m.model):"position"==q.target_property?(q.getSample(c,e.LINEAR,f),mat4.setTranslation(m.model,f)):"rotation"==q.target_property?(q.getSample(c,e.LINEAR,b),mat4.fromQuat(k,b),mat4.getTranslation(f,m.model),mat4.setTranslation(k,f),m.model.set(k)):"scaling"==q.target_property&&
(q.getSample(c,e.LINEAR,f),mat4.scale(m.model,m.model,f)))}};g.prototype.getVertices=function(a,c){if(!this.bones.length)return null;c||this.updateGlobalMatrices();var e=6*(this.bones.length-1);this._vertices&&this._vertices.length==e||(this._vertices=new Float32Array(e));for(var e=this._vertices,b=0,f=1;f<this.bones.length;++f){var g=this.global_bone_matrices[this.bones[f].parent],k=this.global_bone_matrices[f],l=e.subarray(b,b+3),m=e.subarray(b+3,b+6);mat4.getTranslation(l,k);mat4.getTranslation(m,
g);a&&(vec3.transformMat4(l,l,a),vec3.transformMat4(m,m,a));b+=6}return e};g.prototype.resizeBones=function(a){if(this.bones.length!=a)if(this.bones.length>a)this.bones.length=a,this.global_bone_matrices.length=a;else{var c=this.bones.length;for(this.bones.length=a;c<a;++c)this.bones[c]=new f,this.global_bone_matrices[c]=mat4.create()}};g.prototype.copyFrom=function(a){this.resizeBones(a.bones.length);for(var c=0;c<a.bones.length;++c)this.bones[c].copyFrom(a.bones[c]),this.global_bone_matrices[c].set(a.global_bone_matrices[c]);
...
m&&(this.bones_map[f]=m,null==g[l.target_node]&&(g[l.target_node]=l.target_node,f++))}f>a.bones.length&&console.warn("more animated bones than bones?");this.resize(Math.floor(c.duration*e),f);for(k=0;k<this.num_keyframes;++k)this.skeleton.applyTracksAnimation(c,1/e*k),b&&mat4.mul(this.skeleton.bones[0].model,b,this.skeleton.bones[0].model),this.assignPoseToKeyframe(this.skeleton,k)};e.SceneNode&&(e.SceneNode.prototype.assignSkeleton=function(a){var c=gl.meshes[this.mesh];c&&(this.bones=a.computeFinalBoneMatrices(this.bones,
c),this.uniforms.u_bones=this.bones)},e.SceneNode.prototype.assignAnimation=function(a){this.assignSkeleton(a.skeleton)},e.SceneNode.prototype.updateSkinningBones=function(a){a=a||this;this.skin&&this.mesh&&e.collectBones(a,this.skin,gl.meshes[this.mesh]);if(this.children&&this.children.length)for(var c=0;c<this.children.length;++c)this.children[c].updateSkinningBones(a)});e.collectBones=function(a,c,e){function b(a,c,e){if(!c||!a)return e;if(a==c)return mat4.mul(e,a.getGlobalMatrix(),e),e;mat4.mul(e,
a.matrix,e);return b(a._parent,c,e)}if(e&&e.bones){var f=e.bones.length;c._bone_matrices||(c._bone_matrices=new Float32Array(16*f));f=c._bone_matrices;mat4.create();(c=a.findNodeByName(c.skeleton_root))||c==a;for(var g=mat4.create(),k=0;k<e.bones.length;++k){var l=e.bones[k],m=f.subarray(16*k,16*k+16),p=a.findNodeByName(l[0]);p&&(mat4.identity(g),b(p,c,g),mat4.multiply(m,g,l[1]),e.bind_matrix&&mat4.multiply(m,m,e.bind_matrix))}return f}};e.AnimatedCharacterFromScene=function(a,c,f){for(var b=[],g=
[],k=f=null,k=a.constructor===e.SceneNode?a:a.root,l=0;l<k.children.length;++l){var m=k.children[l];if(m.name&&("Armature"==m.name||-1!=m.name.indexOf("_Hips"))||"JOINT"==m.type||m.is_joint)f=m;else if(m.mesh){b.push(m);var p=null;gl.meshes[m.mesh]?p=gl.meshes[m.mesh]:a.meshes&&a.meshes[m.mesh]&&(p=GL.Mesh.load(a.meshes[m.mesh]));p&&g.push({mesh:p})}}!f&&k.findNodesByFilter&&(l=k.findNodesByFilter(function(a){return a.skin}))&&l.length&&(f=l[0]);!b.length&&k.findNodesByFilter&&(b=k.findNodesByFilter(function(a){return a.mesh}));
if(!f)throw"this scene doesnt contain an animated character";l=k=null;b.length&&(m=null,1<b.length?(l=GL.Mesh.mergeMeshes(g),m=b[0].mesh,l.filename=m):(m=b[0].mesh,gl.meshes[m]?(l=gl.meshes[m],l.filename=m):a.meshes&&(l=GL.Mesh.fromBinary(a.meshes[m]),l.filename=m)),gl.meshes[m]=l,g=null,b[0].material?g=b[0].material:b[0].primitives&&b[0].primitives.length&&(g=b[0].primitives[0].material),e.Materials[g]?k=e.Materials[g]:a.materials&&(k=a.materials[g]));b=new e.Skeleton;b.importSkeleton(f,null);g=
null;a.root&&a.root.animation?g=a.root.animation:a.animations&&(g=a.animations[0].id);f=null;e.Animations[g]?f=e.Animations[g]:a.resources&&(a=a.resources[g],f=new e.Animation,f.configure(a.takes["default"]));if(!f)throw"no animation in scene";a=new e.SkeletalAnimation;a.fromTracksAnimation(b,f,30,null);a.filename=c;return{mesh:l?l.filename:null,material:k,skeleton:b,skeletal_anim:a,tracks_anim:f}}})("undefined"!=typeof window?window:"undefined"!=typeof self?self:global);
